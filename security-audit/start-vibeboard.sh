...
#
# Usage:
#   ./start-vibeboard.sh          # Development mode (default)
#   ./start-vibeboard.sh --prod   # Production mode (for Mac Mini deployment)
#   ./start-vibeboard.sh --dev    # Development mode (explicit)
#
...
echo -e "${BLUE}════════════════════════════════════════════════════════════════${NC}"
if [ "$MODE" = "prod" ]; then
    echo -e "${BLUE}  VibeBoard Service Launcher ${RED}[PRODUCTION MODE]${NC}"
else
    echo -e "${BLUE}  VibeBoard Service Launcher ${YELLOW}[DEVELOPMENT MODE]${NC}"
fi
echo -e "${BLUE}════════════════════════════════════════════════════════════════${NC}"
echo ""
...
echo -e "${BLUE}════════════════════════════════════════════════════════════════${NC}"
if [ "$MODE" = "prod" ]; then
    echo -e "${GREEN}All systems operational! ${RED}[PRODUCTION MODE]${NC}"
    echo -e "  ${YELLOW}ComfyUI${NC}:   http://localhost:8188"
    echo -e "  ${YELLOW}Frontend${NC}:  http://localhost:3000 ${RED}(SSR enabled for AI bots)${NC}"
    ...
    echo ""
    echo -e "${RED}PRODUCTION MODE: Frontend is running optimized build with SSR${NC}"
    echo -e "${RED}AI bots (ChatGPT, Google AI Studio) can now access content${NC}"
else
    echo -e "${GREEN}All systems operational! ${YELLOW}[DEVELOPMENT MODE]${NC}"
    echo -e "  ${YELLOW}ComfyUI${NC}:   http://localhost:8188"
...
echo ""
echo -e "${BLUE}Usage: ./start-vibeboard.sh [--prod | --dev]${NC}"
