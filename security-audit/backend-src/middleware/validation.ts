...
export const createGenerationSchema = z.object({
    ...
    aspectRatio: z.string().regex(/^[\d.]+:[\d.]+$/, 'Aspect ratio must be in format "W:H" (e.g., 16:9, 2.35:1)').optional().nullable(),
    variations: z.number().int().min(1).max(8).optional().nullable(),
    duration: z.number().min(1).max(30).optional().nullable(),

    // Advanced parameters
    seed: z.number().int().optional().nullable(),
    guidanceScale: z.number().min(1).max(30).optional().nullable(),
    inferenceSteps: z.number().int().min(1).max(100).optional().nullable(),
    ...
    // References (complex objects, loosely validated)
    usedLoras: z.any().optional(),
...
export const enhancePromptSchema = z.object({
    ...
    characterReferences: z.array(z.any()).optional(),
});
...
export const createTrainingJobSchema = z.object({
    ...
    trainingSteps: z.number().int().min(100).max(10000).optional(),
