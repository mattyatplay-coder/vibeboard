...
export class OpenAIAdapter implements GenerationProvider {
  ...
  async generateImage(options: GenerationOptions): Promise<GenerationResult> {
    try {
      ...
      const outputs = await Promise.all(tempUrls.map(url => this.downloadAndSaveImage(url, model)));
      ...
      if (error.code === 'content_policy_violation') {
        return {
          ...
          status: 'failed',
          error:
            'Content policy violation. Try rephrasing your prompt. Note: DALL-E has strict content filters.',
        };
      }
  ...
  async generateVideo(
    ...
    try {
      ...
      console.log(
        `[OpenAIAdapter] Generating video with model ${model}, size ${size}, duration ${seconds}s`
      );
  ...
  private async downloadAndSaveVideo(id: string): Promise<string> {
    ...
    const response: Response = await (this.client as any).videos.downloadContent(id, {
      variant: 'video',
    });
   ...
   */
  async createVariation(image: Buffer, options: GenerationOptions): Promise<GenerationResult> {
    // Implementation commented out due to File type issues in Node environment
    return {
      id: Date.now().toString(),
      status: 'failed',
