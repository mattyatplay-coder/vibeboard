...
const PROVIDER_CONFIGS: Record<string, ProviderConfig> = {
  ...
  runpod: {
    type: 'runpod',
    name: 'RunPod GPU (Self-Hosted)',
    priority: 0, // Highest priority when explicitly selected
    costPerImage: 0.01, // GPU time cost
    costPerVideo: 0.05, // GPU time for video
    supportsVideo: true,
    supportsImage: true,
    requiresApiKey: true,
    envVar: 'RUNPOD_API_KEY',
    models: {
      image: ['wan-2.1-t2v', 'director-edit', 'lens-character'],
      video: ['wan-2.1-t2v', 'wan-2.1-i2v', 'rack-focus'],
    },
    category: 'cloud',
  },
  ...
  async generateVideo(
    ...
    if (explicitEngine === 'runpod') {
      ...
      try {
        ...
        if (operation === 'rack_focus') {
          result = await gpuClient.rackFocus({
            imageUrl: image || '',
            focusPointStart: (options as any).focusPointStart || [0.5, 0.5],
            focusPointEnd: (options as any).focusPointEnd || [0.5, 0.5],
