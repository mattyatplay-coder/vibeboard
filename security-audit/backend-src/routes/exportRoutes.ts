...
router.post('/master', async (req: Request, res: Response) => {
    try {
        ...
        const { jobId, exportId } = await masterExportService.exportSceneChainAsync(projectId, sceneChainId, {
            exportName,
            format,
            frameRate,
            resolution,
    ...
    try {
        ...
        if (status.status === 'completed' && status.result) {
            res.json({
                ...
                sidecarUrl: status.result.sidecarPath ? `/api/exports/${status.result.exportId}/sidecar` : undefined,
            });
