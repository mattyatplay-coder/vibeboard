...
router.post('/generate-captions', async (req: Request, res: Response) => {
  try {
    ...
    const styleGuide = {
      ...
      dramatic: 'Write evocative, cinematic captions with dramatic flair suitable for a film trailer.',
...
router.post('/export-final', async (req: Request, res: Response) => {
  try {
    ...
    console.log(`[Sequencer] Exporting ${clips.length} clips, format: ${format}, fps: ${frameRate}`);
...
router.post('/export-epk', async (req: Request, res: Response) => {
  try {
    ...
    const avgContinuityScore = continuityScores.reduce((sum, s) => sum + s.score, 0) / continuityScores.length;
