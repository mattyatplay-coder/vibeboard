...
import { YouTubeUploadService, YOUTUBE_CATEGORIES } from '../services/delivery/YouTubeUploadService';
...
router.get('/auth/init', async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        if (!clientId || !clientSecret) {
            res.status(400).json({
                success: false,
                error: 'YouTube API credentials not configured. Set YOUTUBE_CLIENT_ID and YOUTUBE_CLIENT_SECRET in .env',
            });
            return;
        }
...
router.post('/generate-metadata', async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const metadata = await youtubeService.generateMetadata(
            concept,
            archetype,
            hook,
            scriptSummary
