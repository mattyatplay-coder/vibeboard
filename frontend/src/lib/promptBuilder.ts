...
function buildTagEnhancements(tags: Tag[]) {
  ...
  return {
    camera: byCategory['Shots']?.map(t => t.promptKeyword).join(', ') || '',
    lighting: byCategory['Lighting']?.map(t => t.promptKeyword).join(', ') || '',
    style: byCategory['Style']?.map(t => t.promptKeyword).join(', ') || '',
    mood: byCategory['Mood']?.map(t => t.promptKeyword).join(', ') || '',
...
function getEngineOptimalKeywords(engine: VideoEngine): string[] {
  const engineKeywords: Record<string, string[]> = {
    kling: ['cinematic camera movement', 'professional cinematography', 'smooth motion'],
    wan: ['photorealistic', 'high fidelity', 'detailed textures'],
    ltx: ['fast motion', 'dynamic action', 'high frame rate'],
    veo: ['cinematic lighting', 'film grain', 'professional color grading'],
    sora: ['photorealistic', 'natural physics', 'realistic motion'],
    luma: ['creative', 'artistic', 'surreal elements'],
  };
...
function buildNegativePrompt(engine: VideoEngine, _tags: Tag[]): string {
  ...
  const allNegatives = [...baseNegatives, ...(engineNegatives[engine.id] || [])];
