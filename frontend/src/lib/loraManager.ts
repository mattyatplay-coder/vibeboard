...
export function intelligentLoRAApplication(
  ...
  const applicableLoRAs = availableLoRAs.filter(lora => isLoRARelevant(lora, promptAnalysis));
  ...
  for (const lora of applicableLoRAs) {
    ...
    if (!prompt.includes(lora.triggerWord)) {
      enhancedPrompt = insertTriggerWord(enhancedPrompt, lora.triggerWord, lora.category);
    }
  }
...
function isLoRARelevant(lora: ExtendedLoRA, analysis: PromptAnalysis): boolean {
  ...
  switch (lora.category) {
    case 'character':
      ...
      return analysis.hasCharacter && lora.keywords.some(kw => analysis.characterType.includes(kw));

    case 'style':
      // Apply style LoRAs if style matches
      return lora.keywords.some(kw => analysis.style.includes(kw));

    case 'concept':
      // Apply concept LoRAs if subject matches
      return lora.keywords.some(kw => analysis.subject.includes(kw));
