...
export function extractRecommendedSettings(
  ...
  else if (lowerDesc.includes('euler a') || lowerDesc.includes('euler_a'))
    settings.sampler = 'euler_ancestral';
  ...
  else if (lowerDesc.includes('sgm_uniform') || lowerDesc.includes('sgm uniform'))
    settings.scheduler = 'sgm_uniform';
  ...
  const strengthRangeMatch = lowerDesc.match(
    /(?:strength|weight|lora\s*weight):?\s*([\d.]+)\s*[-–to]\s*([\d.]+)/i
  );
  const strengthSingleMatch = lowerDesc.match(
    /(?:strength|weight|lora\s*weight):?\s*([\d.]+)(?!\s*[-–to])/i
  );
