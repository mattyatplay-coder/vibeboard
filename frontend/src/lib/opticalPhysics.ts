...
export const COC_LIMITS: Record<string, number> = {
  'full-frame': 0.03, // Traditional 35mm standard
  'aps-c': 0.02, // Proportional to sensor size
  'aps-c-canon': 0.019,
  super35: 0.02,
...
export function calculateBlurRadius(params: BlurCalculationParams): number {
  const { focalLengthMm, aperture, focusDistanceM, elementDistanceM, sensorType, imageWidthPx } =
    params;
...
export function calculateLayerTransform(params: LayerTransformParams): LayerTransform {
  const { layerDistanceM, focusDistanceM, focalLengthMm, aperture, sensorType, canvasWidthPx } =
    params;
