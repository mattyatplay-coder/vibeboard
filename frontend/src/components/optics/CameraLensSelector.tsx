      ...
      <div className="flex border-b border-white/10">
        {(['presets', 'camera', 'lens'] as const).map(tab => (
          <button
            ...
            className={clsx(
              'flex-1 px-4 py-2.5 text-xs font-medium tracking-wider uppercase transition-colors',
              activeTab === tab
                    ...
                    <div className="space-y-1.5">
                      {brandCameras.map(camera => (
                        <button
                          key={camera.id}
                          onClick={() =>
                            onCameraChange(selectedCamera?.id === camera.id ? null : camera)
                          }
                          className={clsx(
                          ...
                          <div className="flex items-center justify-between">
                            ...
                            <span
                              className={clsx(
                                ...
                                CAMERA_CATEGORY_COLORS[camera.category]
                              )}
                            >
                              {camera.category}
                            </span>
                  ...
                  {filteredLensFamilies.map(family => (
                    ...
                    >
                      ...
                      {family.is_anamorphic && (
                        <span className="ml-1 rounded bg-blue-500/30 px-1 text-[8px] text-blue-300">
                          A
                        </span>
                      )}
                    </button>
        ...
        <div className="border-t border-white/10 bg-black/30 p-3">
          ...
          {selectedCamera && (
            <div className="mt-2 flex items-center gap-2 text-[9px] text-gray-500">
              <span>
                Sensor: {selectedCamera.sensor_spec.width_mm.toFixed(1)}×
                {selectedCamera.sensor_spec.height_mm.toFixed(1)}mm
              </span>
  ...
  return (
    <Tooltip content="Camera & Lens - Professional Optics" side="top">
      ...
      >
        ...
        {selectedLens?.is_anamorphic && (
          <span className="rounded-full bg-blue-500/30 px-1 py-0.5 text-[9px] text-blue-300">
            A
          </span>
        )}
