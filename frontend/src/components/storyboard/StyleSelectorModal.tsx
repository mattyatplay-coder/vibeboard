...
import { X, Upload, Check, ChevronRight, Search, Ratio, Plus, ChevronDown, Settings2, Sliders, Dice5, FileJson, FolderOpen, Library, Database, Lightbulb, Lock } from "lucide-react";
...
export function StyleSelectorModal({ isOpen, onClose, onApply, initialAspectRatio, projectId, config: configProp, currentModelId, isAnamorphicLocked }: StyleSelectorModalProps) {
    // Basic Style State
    ...
    return (
        <>
            <AnimatePresence>
                {isOpen && (
                    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm scrollbar-none" onClick={onClose}>
                        <div className="flex gap-4 items-stretch overflow-hidden scrollbar-none" style={{ height: 'min(800px, calc(100vh - 2rem))', maxWidth: 'calc(100vw - 2rem)' }} onClick={(e) => e.stopPropagation()}>
                            <motion.div
                                initial={{ opacity: 0, scale: 0.95 }}
                                animate={{ opacity: 1, scale: 1 }}
                                exit={{ opacity: 0, scale: 0.95 }}
                                className="relative bg-[#1a1a1a] border border-white/10 rounded-2xl overflow-hidden shadow-2xl flex flex-col h-full flex-shrink-0 scrollbar-none"
                            ...
                            >
                                ...
                                {/* 3-Column Content */}
                                <div className="flex-1 overflow-y-auto flex min-h-0 scrollbar-none">
                                    {/* LEFT COLUMN - Style Presets */}
                                    <div className="w-[280px] border-r border-white/10 flex flex-col scrollbar-none">
                                    ...
                                    <div className="w-[270px] border-r border-white/10 flex flex-col overflow-y-auto scrollbar-none">
                                    ...
                                    <div className="@container w-[230px] flex-shrink-0 flex flex-col overflow-y-auto scrollbar-none">
                                        <div className="p-4 space-y-4">
                                            {/* Quick Add Tags */}
                                            <div>
                                                <button
                                            ...
                                            {/* Aspect Ratio */}
                                            <div>
                                                <span className="text-[clamp(10px,2cqw,12px)] font-bold text-gray-400 uppercase tracking-wider mb-2 block flex items-center gap-[clamp(4px,1cqw,8px)]">
                                                    <Ratio className="w-[clamp(10px,2cqw,12px)] h-[clamp(10px,2cqw,12px)]" /> Aspect Ratio
                                                    {isAnamorphicLocked && (
                                                        <span className="text-[9px] px-1.5 py-0.5 bg-blue-500/20 text-blue-400 rounded ml-1">
                                                            ANAMORPHIC LOCK
                                                        </span>
                                                    )}
                                                </span>
                                                {isAnamorphicLocked ? (
                                                    ...
                                                    <div className="flex items-center gap-2">
                                                        <div className="px-[clamp(8px,2.5cqw,14px)] py-[clamp(4px,1.2cqw,8px)] rounded-lg text-[clamp(10px,2cqw,12px)] font-medium bg-blue-600 border border-blue-500 text-white whitespace-nowrap">
                                                            21:9
                                                        </div>
                                                    ...
                                                    <div className="flex flex-wrap gap-[clamp(4px,1.5cqw,8px)]">
                                                        {['16:9', '9:16', '1:1', '21:9', '2.35:1'].map((ratio) => (
                                                            <button
                                                                ...
                                                                className={clsx(
                                                                    "px-[clamp(8px,2.5cqw,14px)] py-[clamp(4px,1.2cqw,8px)] rounded-lg text-[clamp(10px,2cqw,12px)] font-medium transition-colors border whitespace-nowrap",
                                            ...
                                            <div className="bg-amber-500/5 border border-amber-500/20 rounded-lg p-3">
                                                <div className="flex items-center justify-between mb-2">
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        <span className="text-xs font-bold text-amber-300 uppercase tracking-wider">Lighting Lock</span>
                                                    </div>
                                                    <button
                                                        onClick={() => setConfig({
                                                            ...config,
                                                ...
                                                <p className="text-[10px] text-gray-500 mb-2">
                                                    Upload a reference image to lock lighting style across all generations (uses IP-Adapter at low weight)
                                                </p>
                                                ...
                                                {config.lightingLock?.enabled && (
                                                    <div className="space-y-2">
                                                        ...
                                                        {config.lightingLock.imageUrl || config.lightingLock.imageFile ? (
                                                            <div className="relative w-full h-20 rounded-lg overflow-hidden border border-amber-500/30">
                                                                <img
                                                                    src={config.lightingLock.imageFile
                                                                ...
                                                                <button
                                                                    onClick={() => setConfig({
                                                                        ...config,
                                                                        lightingLock: { ...config.lightingLock!, imageUrl: undefined, imageFile: undefined }
                                                                    })}
                                                                    className="absolute top-1 right-1 p-1 rounded bg-black/50 hover:bg-red-500/50 transition-colors"
                                                                >
                                                                    <X className="w-3 h-3 text-white" />
                                                                </button>
                                                            </div>
                                                        ) : (
                                                            <label className="flex flex-col items-center justify-center w-full h-16 border-2 border-dashed border-amber-500/30 rounded-lg cursor-pointer hover:border-amber-500/50 transition-colors bg-amber-500/5">
                                                                <Upload className="w-4 h-4 text-amber-400 mb-1" />
                                                                <span className="text-[10px] text-amber-300">Drop lighting reference</span>
                                                                <input
                                                                    type="file"
                                                                    accept="image/*"
                                                                    className="hidden"
                                                        ...
                                                        {/* Strength Slider */}
                                                        <div className="flex items-center gap-2">
                                                            <span className="text-[10px] text-gray-500 w-14">Strength:</span>
                                                            <input
                                                                type="range"
                                                                min="0.1"
                                                                max="0.5"
                                                                ...
                                                                onChange={(e) => setConfig({
                                                                    ...config,
                                                                    lightingLock: { ...config.lightingLock!, strength: parseFloat(e.target.value) }
                                                                })}
                                                                className="flex-1 h-1 bg-gray-700 rounded-full appearance-none cursor-pointer [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-3 [&::-webkit-slider-thumb]:h-3 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-amber-500"
                                                            />
                                                            <span className="text-[10px] text-amber-300 w-8 text-right">
                            ...
                            <AnimatePresence>
                                {activeManager === 'lora' && (
                                    <motion.div
                                        ...
                                        exit={{ opacity: 0, x: -20 }}
                                        className="h-full max-h-full flex-shrink min-w-[400px]"
                                    >
                                        <LoRAManager
                                            projectId={projectId}
                                            isOpen={true}
                                ...
                                {activeManager === 'sampler' && (
                                    <motion.div
                                        ...
                                        animate={{ opacity: 1, x: 0 }}
                                        exit={{ opacity: 0, x: -20 }}
                                        className="h-full max-h-full flex-shrink min-w-[300px]"
                                    >
                                        <ParameterManager
                                            projectId={projectId}
                                            type="sampler"
                                ...
                                {activeManager === 'negative' && (
                                    <motion.div
                                        ...
                                        animate={{ opacity: 1, x: 0 }}
                                        exit={{ opacity: 0, x: -20 }}
                                        className="h-full max-h-full flex-shrink min-w-[350px]"
                                    >
