...
// Qwen-2511 is the strategic default for I2I (surgical editing with identity preservation)
// Flux Kontext is for contextual inpainting/outpainting (more hallucination-friendly)
const IMAGE_MODELS = [
  {
    id: 'fal-ai/qwen-image-edit-2511',
    name: 'Qwen 2511 (I2I)',
    desc: 'Surgical editing, identity preservation',
  },
  { id: 'fal-ai/flux-kontext/dev', name: 'Flux Kontext Dev', desc: 'Contextual inpainting' },
  { id: 'fal-ai/flux-kontext/pro', name: 'Flux Kontext Pro', desc: 'Premium contextual editing' },
...
function getVideoResolutions(videoModel: string | null | undefined) {
  const model = videoModel || 'runpod/stable-video-infinity';
...
const VIDEO_MODELS = [
  {
    ...
    desc: 'Long-form continuity, 100+ frames',
  },
  {
  ...
  },
  {
    ...
    name: 'Kling 2.6 Pro',
    desc: 'Latest Kling with pro features',
  },
  {
    id: 'fal-ai/wan/v2.6/image-to-video',
    name: 'Wan 2.6 I2V',
    desc: 'Alibaba latest, great motion',
  },
  ...
  { id: 'fal-ai/ltx-video/image-to-video', name: 'LTX Video', desc: 'Fast conversion' },
  {
    ...
    name: 'Hunyuan I2V',
    desc: 'Open-source, diverse motion',
  },
...
export interface ShotData {
  ...
  negativePrompt?: string | null; // For surgical removal of unwanted elements in I2I (e.g., "(two figures on sidewalk:1.5)")
...
interface StoryboardShotProps {
  ...
  prevShotLastFrameUrl?: string | null; // Previous shot's last frame for Quick Look continuity preview
...
export default function StoryboardShot({
  ...
  // Quick Look: Hover state for showing previous shot's last frame
  const [isQuickLookActive, setIsQuickLookActive] = useState(false);
  ...
  const [shotPopoutPosition, setShotPopoutPosition] = useState<{
    top: number;
    left: number;
  } | null>(null);
      ...
      <div className="flex gap-4">
        ...
        <div className="flex w-[420px] flex-shrink-0 flex-col rounded-xl border border-white/10 bg-[#1a1a1a] p-4">
          ...
          <div className="relative mb-3">
            <div className="mb-1 flex items-center justify-between">
              <span className="text-[10px] font-medium tracking-wide text-green-400 uppercase">
                First Frame Prompt
              </span>
            ...
            {expandedPrompt === 'first' && (
              <div className="animate-in fade-in zoom-in-95 absolute top-0 left-0 z-50 w-[600px] duration-150">
                <div className="relative rounded-xl border border-green-500/50 bg-[#1a1a1a] p-3 shadow-2xl shadow-black/50">
                  ...
                  <AutocompletePopup
                    ...
                    className="right-0 bottom-full left-0 mb-2"
                  />
                  <textarea
                    autoFocus
                    value={editingFirstPrompt}
                    ...
                    rows={Math.max(
                      5,
                      editingFirstPrompt.split('\n').length +
                        Math.ceil(editingFirstPrompt.length / 80)
                    )}
          ...
          <div className="relative mb-3">
            <div className="mb-1 flex items-center justify-between">
              <span className="text-[10px] font-medium tracking-wide text-purple-400 uppercase">
                Last Frame Prompt
              </span>
              ...
              <div className="animate-in fade-in zoom-in-95 absolute top-0 left-0 z-50 w-[600px] duration-150">
                <div className="relative rounded-xl border border-purple-500/50 bg-[#1a1a1a] p-3 shadow-2xl shadow-black/50">
                  ...
                  <textarea
                    ...
                    rows={Math.max(
                      5,
                      editingLastPrompt.split('\n').length +
                        Math.ceil(editingLastPrompt.length / 80)
                    )}
          ...
          <div className="mb-3">
            ...
            <div className="relative">
              <select
                value={shot.imageModel || 'fal-ai/qwen-image-edit-2511'}
            ...
            <div className="mt-1 flex items-center justify-between gap-2">
              <p className="text-[10px] text-gray-500">
                {IMAGE_MODELS.find(m => m.id === (shot.imageModel || 'fal-ai/qwen-image-edit-2511'))
                  ?.desc || ''}
              </p>
              ...
              <div className="flex shrink-0 items-center gap-1.5">
                ...
                <div className="relative">
                  ...
                  <ChevronDown className="pointer-events-none absolute top-1/2 right-1 h-3 w-3 -translate-y-1/2 text-gray-500" />
                </div>
              </div>
            </div>
          </div>

          {/* Negative Prompt - for surgical removal of unwanted elements in I2I */}
          <div className="mt-3">
            <div className="mb-1 flex items-center gap-1.5">
              <span className="text-[10px] font-medium tracking-wide text-red-400 uppercase">
                Negative Prompt
              </span>
              <Tooltip
                content="Elements to surgically remove from I2I edits (e.g., 'two figures on sidewalk:1.5')"
                side="top"
              >
                <span className="cursor-help text-[9px] text-gray-500">(?)</span>
              </Tooltip>
            </div>
            <input
              type="text"
              value={shot.negativePrompt || ''}
              onChange={e => onUpdate(shot.id, { negativePrompt: e.target.value })}
              placeholder="(person in background:1.5), crowd, blurry figures..."
              className="w-full rounded-lg border border-white/10 bg-black/40 px-2.5 py-1.5 text-xs text-white placeholder-gray-600 transition-colors hover:border-red-500/30 focus:border-red-500/50 focus:outline-none"
            />
            <p className="mt-0.5 text-[9px] text-gray-500">
              Use weighted terms like (element:1.5) for stronger removal
            </p>
          </div>
          ...
          <div className="mt-4 grid grid-cols-2 gap-3">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="hidden"
              />
              {/* First Frame Preview with Quick Look continuity feature */}
              <div
                onClick={() => {
                ...
                onMouseEnter={() => prevShotLastFrameUrl && setIsQuickLookActive(true)}
                onMouseLeave={() => setIsQuickLookActive(false)}
                className={clsx(
              ...
              >
                {isGeneratingFirstFrame ? (
                  ...
                  <>
                    {/* Current shot's first frame (base layer) */}
                    <img
                      src={resolveUrl(shot.firstFrameUrl)}
                      alt="Beginning frame"
                      className="absolute inset-0 h-full w-full object-cover"
                    />
                    {/* Quick Look: Previous shot's last frame (overlay on hover) */}
                    {isQuickLookActive && prevShotLastFrameUrl && (
                      <>
                        <img
                          src={resolveUrl(prevShotLastFrameUrl)}
                          alt="Previous shot's last frame"
                          className="absolute inset-0 z-10 h-full w-full object-cover transition-opacity duration-300"
                        />
                        {/* Quick Look badge */}
                        <span className="absolute right-1 bottom-1 z-20 rounded bg-black/70 px-1.5 py-0.5 text-[9px] font-medium text-amber-400 backdrop-blur-sm">
                          Quick Look
                        </span>
                      </>
                    )}
        ...
        <div className="min-w-[700px] flex-1 overflow-hidden rounded-xl border border-white/10 bg-[#1a1a1a]">
          ...
          >
            ...
            {/* Preview Content */}
            <div
              className={clsx('aspect-video', shot.outputUrl && 'cursor-zoom-in')}
              onClick={() => {
                if (shot.outputUrl) {
          ...
          <div className="flex items-end gap-3 border-t border-white/5 bg-black/40 px-4 py-3">
            ...
            <div>
              ...
              <div className="mb-1 flex items-center gap-1.5">
                ...
                <span className="text-[10px] font-medium tracking-wide text-cyan-400 uppercase">
                  Video Model
                </span>
              </div>
              <div className="flex items-center gap-2">
                <Tooltip
                  content={
                    VIDEO_MODELS.find(
                      m => m.id === (shot.videoModel || 'runpod/stable-video-infinity')
                    )?.desc || ''
                  }
                  side="top"
                >
                  <div className="relative">
                    <select
                      value={shot.videoModel || 'runpod/stable-video-infinity'}
            ...
            <div className="flex items-end gap-1.5">
              ...
              {(shot.videoIterations || 0) > 0 && (
                <span className="pb-1.5 text-[10px] text-cyan-400">
                  ...
                  <span className="ml-0.5 text-gray-500">({shot.videoIterations}×)</span>
                </span>
              )}
            </div>
          </div>
          ...
          {(shot.firstFrameIterations || 0) +
            (shot.lastFrameIterations || 0) +
            (shot.videoIterations || 0) >
            0 && (
                ...
                <div className="flex items-center gap-3">
                  {(shot.firstFrameIterations || 0) + (shot.lastFrameIterations || 0) > 0 && (
                    <span className="text-amber-400/80">
                      {(shot.firstFrameIterations || 0) + (shot.lastFrameIterations || 0)} frames:{' '}
                      {formatCost(
                        ((shot.firstFrameIterations || 0) + (shot.lastFrameIterations || 0)) *
                          calculateImageCost(shot.imageModel, shot.imageResolution)
                      )}
                  ...
                  {(shot.videoIterations || 0) > 0 && (
                    <span className="text-emerald-400/80">
                      {shot.videoIterations} video{(shot.videoIterations || 0) > 1 ? 's' : ''}:{' '}
                      {formatCost(
                        (shot.videoIterations || 0) *
                          calculateVideoCost(
                            shot.videoModel,
          ...
          {expandedPrompt === 'shot' &&
            ...
            createPortal(
              <div
                className="animate-in fade-in zoom-in-95 fixed z-[9999] w-[700px] duration-150"
              ...
              >
                <div className="relative rounded-xl border border-cyan-500/50 bg-[#1a1a1a] p-3 shadow-2xl shadow-black/50">
                  <div className="mb-2 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <select
                        value={shot.duration}
                        onChange={e =>
                          onUpdate(shot.id, { duration: parseInt(e.target.value) || 5 })
                        }
                  ...
                  <textarea
                    ...
                    rows={Math.max(
                      6,
                      editingShotPrompt.split('\n').length +
                        Math.ceil(editingShotPrompt.length / 90)
                    )}
          ...
          >
            ...
            >
              {lightboxOpen === 'video' ? (
                ...
                <img
                  src={resolveUrl(
                    lightboxOpen === 'first' ? shot.firstFrameUrl : shot.lastFrameUrl
                  )}
            ...
            <div className="absolute bottom-4 left-1/2 -translate-x-1/2 text-xs text-gray-500">
              Press <kbd className="rounded border border-gray-600 px-1.5 py-0.5">Esc</kbd> or click
              outside to close
            </div>
          </div>,
          document.body
