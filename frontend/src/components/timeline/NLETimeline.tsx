...
function snapToPoint(
  ...
  threshold: number,
  zoom: number
): { value: number; snapped: boolean; snapTarget?: number } {
...
const VIDEO_CLIP_STYLES = {
  ...
  selected:
    'bg-gradient-to-b from-cyan-400/95 to-cyan-500/90 ring-2 ring-white shadow-[0_0_15px_rgba(255,255,255,0.3)]',
};
...
function VideoClipComponent({
  ...
  return (
    <div
      ref={clipRef}
      className={`group absolute top-0 h-full cursor-pointer overflow-hidden rounded-md transition-all duration-200 ${
        isSelected
    ...
    >
      ...
      <div className="relative z-10 flex h-full flex-col justify-between p-2">
        <span className="truncate text-[10px] font-bold text-white/90 drop-shadow-md">
          {clip.name}
        </span>

        {/* Waveform / Thumbnails placeholder */}
        <div className="min-h-[12px] w-full flex-1 rounded-sm bg-black/10" />
      </div>

      {/* Duration badge - glassmorphic */}
      <div className="absolute right-1.5 bottom-1 rounded-md bg-black/40 px-1.5 py-0.5 font-mono text-[9px] text-white/70 backdrop-blur-sm">
        {clipDuration.toFixed(2)}s
      </div>

      {/* 3. Drag Handles (Visible on Hover) - Glass style */}
      <div
        className={`absolute top-0 bottom-0 left-0 w-3 cursor-ew-resize transition-opacity ${
          isDraggingStart
      ...
      />
      <div
        className={`absolute top-0 right-0 bottom-0 w-3 cursor-ew-resize transition-opacity ${
          isDraggingEnd
            ? 'bg-white/30 opacity-100'
            : 'bg-white/10 opacity-0 group-hover:opacity-100'
        }`}
...
const AUDIO_CLIP_STYLES = {
  ...
  selected:
    'bg-gradient-to-b from-violet-400/95 to-violet-500/90 ring-2 ring-white shadow-[0_0_15px_rgba(255,255,255,0.3)]',
};
  ...
  return (
    ...
    >
      ...
      <button
        ...
        className={`absolute top-1 left-1.5 z-10 rounded-full p-0.5 ${
      ...
      {offsetBadge && (
        <div
          className={`absolute top-1 right-1.5 z-10 rounded-md px-1.5 py-0.5 text-[9px] font-medium text-white backdrop-blur-sm ${offsetBadge.color}/80`}
        >
          {offsetBadge.text}
        </div>
      )}
...
export function NLETimeline({
  ...
  const handleTrimStart = useCallback(
    (clipId: string, delta: number, snapped?: boolean, snapTarget?: number) => {
      ...
      onClipsChange(
        clips.map(clip => {
          ...
          const newTrimEnd = Math.max(
            clip.trimStart + 0.1,
            Math.min(clip.trimEnd + delta, clip.duration)
          );
          return {
            ...clip,
            trimEnd: newTrimEnd,
  ...
  const handleAudioTrimStart = useCallback(
    (clipId: string, delta: number, snapped?: boolean, snapTarget?: number) => {
      ...
      onClipsChange(
        clips.map(clip => {
          if (clip.id !== clipId) return clip;
          const newAudioTrimStart = Math.max(
            0,
            Math.min(clip.audioTrimStart + delta, clip.audioTrimEnd - 0.1)
          );
  ...
  const handleAudioTrimEnd = useCallback(
    (clipId: string, delta: number, snapped?: boolean, snapTarget?: number) => {
      ...
      onClipsChange(
        clips.map(clip => {
          if (clip.id !== clipId) return clip;
          const newAudioTrimEnd = Math.max(
            clip.audioTrimStart + 0.1,
            Math.min(clip.audioTrimEnd + delta, clip.duration)
          );
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden rounded-xl border border-white/5 bg-zinc-900/80 backdrop-blur-sm">
      ...
      <div className="flex items-center justify-between border-b border-white/5 bg-zinc-800/40 px-4 py-2 backdrop-blur-sm">
        ...
        {/* Timecode display */}
        <div className="font-mono text-sm text-white">{formatTimecode(currentTime, frameRate)}</div>
        ...
        {markers.length > 0 && (
          <div className="flex items-center gap-1.5 rounded-full border border-amber-500/30 bg-amber-500/10 px-2 py-0.5">
            <div className="h-0 w-0 border-t-[6px] border-r-[4px] border-l-[4px] border-t-amber-400 border-r-transparent border-l-transparent" />
      ...
      <div ref={scrollContainerRef} className="flex-1 overflow-x-auto overflow-y-hidden">
        <div
          ...
          style={{
            width: timelineWidth,
            minHeight: RULER_HEIGHT + TRACK_HEIGHT + AUDIO_TRACK_HEIGHT + 40,
          }}
          onClick={handleTimelineClick}
        >
          {/* Cinematic Ruler - with tick marks and visual density */}
          <div
            className="sticky top-0 z-20 flex items-end border-b border-white/5 bg-zinc-950/50"
          ...
          >
            {rulerMarks.map((mark, i) => (
              ...
              >
                {/* Major Tick (Second) */}
                <div
                  className={`${mark.isMajor ? 'h-3 w-px bg-zinc-500' : 'h-2 w-px bg-zinc-700'}`}
                />
                {mark.isMajor && (
                  <span className="absolute bottom-4 left-1 font-mono text-[9px] font-medium text-zinc-500 select-none">
                    {mark.label}
                  </span>
                )}
                ...
                {mark.isMajor && zoom > 30 && (
                  <>
                    <div
                      className="absolute bottom-0 left-[25%] h-1.5 w-px bg-zinc-800"
                      style={{ left: zoom * 0.25 }}
                    />
                    <div
                      className="absolute bottom-0 left-[50%] h-1.5 w-px bg-zinc-800"
                      style={{ left: zoom * 0.5 }}
                    />
                    <div
                      className="absolute bottom-0 left-[75%] h-1.5 w-px bg-zinc-800"
                      style={{ left: zoom * 0.75 }}
                    />
                  </>
                )}
              </div>
            ...
            {markers.map(marker => (
              ...
              >
                ...
                >
                  {/* Triangle marker */}
                  <div
                    className="h-0 w-0 border-t-[10px] border-r-[6px] border-l-[6px] border-r-transparent border-l-transparent transition-transform hover:scale-110"
                    style={{ borderTopColor: marker.color }}
                  />
                  {/* Vertical line */}
                  <div
                    className="absolute top-[10px] left-1/2 h-[14px] w-0.5 -translate-x-1/2"
                    style={{ backgroundColor: marker.color }}
                  />
                  {/* Label tooltip on hover */}
                  {marker.label && (
                    <div className="absolute top-7 left-1/2 z-50 hidden -translate-x-1/2 rounded bg-zinc-800 px-2 py-1 text-xs whitespace-nowrap text-white shadow-lg group-hover:block">
                      {marker.label}
                    </div>
                  )}
                  {/* Delete button on hover */}
                  <button
                    className="absolute -top-1 -right-4 hidden h-4 w-4 items-center justify-center rounded-full bg-red-500 text-xs text-white group-hover:flex"
                    onClick={e => {
                      e.stopPropagation();
          ...
          <div className="absolute top-0 left-0 z-30 w-12 bg-zinc-900/90">
            <div style={{ height: RULER_HEIGHT }} className="border-r border-b border-white/10" />
            <Tooltip content="Video Track - Drag edges to trim, snap to other clips" side="right">
              <div
                className="flex cursor-help flex-col items-center justify-center gap-0.5 border-r border-b border-white/10 bg-gradient-to-r from-cyan-950/50 to-transparent transition-colors hover:from-cyan-900/50"
          ...
          >
            {clipPositions.map(({ clip, startTime }) => (
              <VideoClipComponent
                ...
                onTrimStart={(delta, snapped, snapTarget) =>
                  handleTrimStart(clip.id, delta, snapped, snapTarget)
                }
                onTrimEnd={(delta, snapped, snapTarget) =>
                  handleTrimEnd(clip.id, delta, snapped, snapTarget)
                }
                onMove={() => {}}
              />
            ))}
          ...
          {/* Audio track (A1) */}
          <div className="relative ml-12 bg-zinc-800/20" style={{ height: AUDIO_TRACK_HEIGHT }}>
            {clipPositions.map(({ clip, startTime }) => (
              <AudioClipComponent
                key={`audio-${clip.id}`}
                clip={clip}
                ...
                snapPoints={findSnapPoints(clips, clip.id, currentTime, markIn, markOut)}
                onSelect={() => setSelectedClipId(clip.id)}
                onAudioTrimStart={(delta, snapped, snapTarget) =>
                  handleAudioTrimStart(clip.id, delta, snapped, snapTarget)
                }
                onAudioTrimEnd={(delta, snapped, snapTarget) =>
                  handleAudioTrimEnd(clip.id, delta, snapped, snapTarget)
                }
          ...
          <AnimatePresence>
            {snapIndicator !== null && (
              ...
              >
                ...
                <div className="absolute -top-1 -left-1 h-2 w-2 rounded-full bg-green-400" />
                {/* Snap label */}
                <div className="absolute top-1 -left-6 rounded bg-green-500/80 px-1 text-[9px] font-medium text-white">
                  SNAP
                </div>
              </motion.div>
            )}
      ...
      <div className="flex items-center justify-between border-t border-white/5 bg-zinc-800/40 px-4 py-1.5 text-xs text-white/50 backdrop-blur-sm">
        ...
        <div className="flex items-center gap-4">
          {markIn !== null && (
            <span className="text-yellow-400/70">In: {formatTimecode(markIn, frameRate)}</span>
          )}
          {markOut !== null && (
            <span className="text-yellow-400/70">Out: {formatTimecode(markOut, frameRate)}</span>
          )}
          {markIn !== null && markOut !== null && (
            <span className="text-yellow-300">
              Selection: {formatTimecode(markOut - markIn, frameRate)}
            </span>
          )}
        </div>
      </div>
