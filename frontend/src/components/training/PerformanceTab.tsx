    ...
    <div className="animate-in fade-in space-y-6">
      ...
      <div className="grid grid-cols-2 gap-8">
        ...
        <div className="space-y-4">
          <label className="flex items-center gap-2 text-sm font-bold tracking-wider text-gray-400 uppercase">
          ...
          {characters.length === 0 ? (
            <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-white/20 py-12">
              ...
              <p className="mt-1 text-xs text-gray-600">
                Add characters via Script Lab or Elements
              </p>
            </div>
          ) : (
            <div className="scrollbar-thin scrollbar-thumb-white/10 grid max-h-60 grid-cols-3 gap-3 overflow-y-auto pr-2">
              {characters.map(char => {
                  ...
                  <button
                    ...
                    className={clsx(
                      ...
                      selectedChar === char.id
                        ? 'scale-[1.02] border-purple-500 ring-2 ring-purple-500/30'
                        : 'border-white/10 hover:border-white/30'
                    )}
                  >
                    {hasImage ? (
                      <img src={imageUrl} alt={char.name} className="h-full w-full object-cover" />
                    ) : (
        ...
        <div className="space-y-4">
          ...
          <div className="scrollbar-thin scrollbar-thumb-white/10 max-h-60 space-y-2 overflow-y-auto pr-2">
            ...
            <label
              className={clsx(
                'flex w-full cursor-pointer items-center justify-center gap-2 rounded-lg border border-dashed py-4 transition-all',
                uploadingAudio
                  ? 'border-cyan-500/50 bg-cyan-500/10 text-cyan-400'
                  : 'border-white/20 text-gray-500 hover:border-white/40 hover:bg-white/5 hover:text-white'
              )}
            >
              <input
                type="file"
      ...
      <div className="border-t border-white/10 pt-4">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            {selectedCharacter && selectedAudioFile && <span className="text-gray-600">+</span>}
            {selectedAudioFile && (
          ...
          <button
            ...
            className={clsx(
              'flex items-center gap-2 rounded-xl px-8 py-4 font-bold text-white shadow-lg transition-all',
