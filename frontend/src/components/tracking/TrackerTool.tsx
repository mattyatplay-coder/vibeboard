...
export default function TrackerTool({
  ...
  const handleCanvasMouseMove = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    setCorners(prev =>
      prev.map((corner, index) => (index === selectedCorner ? { ...corner, x, y } : corner))
    );
      ...
      <div className="relative flex-1 overflow-hidden">
        ...
        {isTracking && (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/70">
            ...
            <div className="mb-2 text-lg font-medium text-white">Tracking Points...</div>
            ...
            <div className="mt-2 text-sm text-gray-400">{trackingProgress}% complete</div>
          </div>
        )}
      </div>
          ...
          {corners.map((corner, idx) => (
            <div
              ...
              className={clsx(
                ...
                selectedCorner === idx ? 'bg-cyan-500/20 text-cyan-400' : 'bg-white/5 text-gray-400'
              )}
            >
