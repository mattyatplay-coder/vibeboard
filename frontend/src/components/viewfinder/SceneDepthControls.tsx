...
function LayerItem({
  ...
  const focusStatus = isInFocus
    ? 'In Focus'
    : layer.distanceM < focusDistance
      ? 'Foreground'
      : 'Background';
      ...
      >
        ...
        {layer.imageUrl && (
          <button
            ...
            className="rounded p-0.5 text-gray-500 transition-colors hover:bg-red-500/20 hover:text-red-400"
          ...
          >
            <div className="space-y-3 p-2">
              ...
              <div className="flex flex-wrap gap-1">
                {Object.entries(STANDARD_FRAMING_DISTANCES)
                  .slice(0, 5)
                  .map(([key, dist]) => (
                    <button
                      key={key}
    ...
    <div className="space-y-3 rounded-lg border border-white/10 bg-black/40 p-3">
      ...
      <div className="rounded bg-green-500/10 p-2">
        <div className="flex items-center justify-between">
          ...
          <span className="text-[9px] text-white">
            {dofLimits.nearLimitM.toFixed(2)}m -{' '}
            {dofLimits.isHyperfocal ? '∞' : `${dofLimits.farLimitM.toFixed(2)}m`}
          </span>
        </div>
      ...
      {layers.length > 0 && (
        <div className="border-t border-white/10 pt-3">
          ...
          <div className="relative h-8 rounded bg-gradient-to-r from-purple-500/30 via-green-500/30 to-amber-500/30">
            ...
            <div className="absolute right-0 -bottom-4 text-[7px] text-gray-500">20m</div>
          </div>
        </div>
      )}
    </div>
