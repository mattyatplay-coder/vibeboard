...
interface DirectorViewfinderProps {
  ...
  camera?: CameraSpec | null; // Selected camera for look profile color science
}
...
const FRAMING_PRESETS: FramingPreset[] = [
  ...
  },
  {
    id: 'medium',
    name: 'Medium',
    description: 'Waist up, classic interview',
    modelDistance: 2.0,
    modelScale: 1.5,
  },
  {
    id: 'american',
    name: 'American',
    description: 'Mid-thigh up, Western framing',
    modelDistance: 3.0,
    modelScale: 1.2,
  },
  {
...
function getLensCompressionDescription(focalLengthMm: number): string {
  if (focalLengthMm < 24)
    return 'Ultra-wide - exaggerated perspective, objects appear more distant';
  if (focalLengthMm < 35)
    return 'Wide - slight perspective exaggeration, good for environmental context';
  ...
  if (focalLengthMm < 200)
    return 'Telephoto - noticeable compression, subjects appear closer together';
...
function DOFVisualizer({ settings, className }: { settings: DOFSettings; className?: string }) {
  ...
  return (
    <div
      className={clsx(
        'relative h-8 rounded-lg bg-gradient-to-r from-gray-800 via-gray-700 to-gray-800',
        className
      )}
    >
      ...
      {/* In-focus zone */}
      <div
        className="absolute inset-y-0 border-x border-green-500/50 bg-green-500/30"
        style={{
          left: `${nearPos}%`,
      ...
      <div
        className="absolute top-1/2 h-3 w-3 -translate-x-1/2 -translate-y-1/2 rounded-full border-2 border-green-400 bg-white shadow-lg shadow-green-500/50"
        style={{ left: `${focusPos}%` }}
      />

      {/* Labels */}
      <div className="absolute -bottom-5 left-0 text-[9px] text-gray-500">Near</div>
      <div className="absolute right-0 -bottom-5 text-[9px] text-gray-500">Far</div>
      <div
...
function DOFLayeredScene({
  ...
  return (
    <div className="absolute inset-0 overflow-hidden">
      ...
      >
        {bgLayer?.imageUrl ? (
          <img src={bgLayer.imageUrl} alt="Background" className="h-full w-full object-cover" />
        ) : referenceImageUrl ? (
          <img src={referenceImageUrl} alt="Background" className="h-full w-full object-cover" />
        ) : (
        ...
        <div className="absolute right-2 bottom-2 rounded bg-black/60 px-2 py-1 text-[9px] text-purple-400">
          BG: {actualBgBlur.toFixed(1)}px blur @{' '}
          {bgLayer?.distanceM.toFixed(1) ?? backgroundDistance}m
        </div>
      </div>
      ...
      >
        {subjectLayer?.imageUrl ? (
          <img src={subjectLayer.imageUrl} alt="Subject" className="h-full w-full object-contain" />
        ) : (
        ...
        <div className="absolute bottom-2 left-1/2 -translate-x-1/2 rounded bg-black/60 px-2 py-1 text-[9px] text-green-400">
          FOCUS: {subjectLayer?.distanceM.toFixed(1) ?? focusDistanceM.toFixed(1)}m •{' '}
          {actualSubjectBlur < 1 ? 'Sharp' : `${actualSubjectBlur.toFixed(1)}px blur`}
        </div>
      </div>
      ...
      {(fgLayer?.imageUrl || settings.aperture <= 4) && (
        ...
        >
          {fgLayer?.imageUrl ? (
            <img src={fgLayer.imageUrl} alt="Foreground" className="h-full w-full object-contain" />
          ) : (
          ...
          <div className="absolute bottom-2 left-2 rounded bg-black/60 px-2 py-1 text-[9px] text-cyan-400">
            FG: {actualFgBlur.toFixed(1)}px blur @{' '}
            {fgLayer?.distanceM.toFixed(1) ?? foregroundDistance}m
          </div>
        </div>
      )}

      {/* FOV Indicator Overlay */}
      <div className="absolute top-2 left-2 space-y-1 rounded bg-black/60 px-2 py-1.5">
        <div className="text-[9px] text-gray-400">Field of View</div>
        <div className="flex items-center gap-2">
          <div className="font-mono text-xs text-amber-400">{aov.horizontal}°</div>
          <span className="text-[8px] text-gray-500">horizontal</span>
        </div>
        <div className="text-[8px] text-gray-500">
          {getLensCompressionDescription(settings.focalLength).split(' - ')[0]}
        </div>
      </div>
    </div>
  );
  ...
  return (
    ...
    >
      <img
        ...
        alt={element.name}
        className="pointer-events-none max-h-32 max-w-32 object-contain"
        draggable={false}
      />
      {isSelected && !element.locked && (
        <div className="absolute -top-6 left-1/2 -translate-x-1/2 rounded bg-cyan-500/90 px-2 py-0.5 text-[10px] whitespace-nowrap text-white">
          {element.name}
        </div>
      )}
    </div>
...
export function DirectorViewfinder({
  ...
  const [selectedFraming, setSelectedFraming] = useState<FramingPreset | null>(
    FRAMING_PRESETS.find(f => f.id === 'medium') || null
  );
  const [selectedCameraOld, setSelectedCameraOld] = useState<CameraModel | null>(null); // Legacy - for old dropdown
  const [isExtractingLayers, setIsExtractingLayers] = useState(false);

  // Camera/Lens Selector Modal State (replaces old camera dropdown)
  const [isCameraLensSelectorOpen, setIsCameraLensSelectorOpen] = useState(false);
  const [selectedCameraSpec, setSelectedCameraSpec] = useState<CameraSpec | null>(camera);
  ...
  const [extractedLayers, setExtractedLayers] = useState<{
    subject?: string;
    background?: string;
    foreground?: string;
  } | null>(null);

  // Sync camera prop with local state (when parent updates camera selection)
  useEffect(() => {
    setSelectedCameraSpec(camera);
  }, [camera]);
  ...
  // Calculate FOV/AOV
  const aovResult = useMemo(
    () => calculateAOV(dofSettings.focalLength, dofSettings.sensorSize),
    [dofSettings.focalLength, dofSettings.sensorSize]
  );
  ...
  // Apply camera preset - updates sensor size (legacy function for old CameraModel type)
  const applyCameraPreset = useCallback((cam: CameraModel) => {
    setSelectedCameraOld(cam);
    setDofSettings(prev => ({ ...prev, sensorSize: cam.sensorSize }));
  }, []);
  ...
  const handleManualLayerUpload = useCallback(
    async (layerType: 'subject' | 'background' | 'foreground', file: File) => {
      try {
  ...
  useEffect(() => {
    if (
      extractedLayers &&
      (extractedLayers.subject || extractedLayers.background || extractedLayers.foreground)
    ) {
  ...
  const generateDOFPrompt = useCallback(() => {
    ...
    if (selectedLens) {
      if (dofSettings.sensorSize !== 'full-frame') {
        parts.push(
          `shot on ${selectedLens.focalLength} lens (${dofResult.equivalentFocalLength}mm equivalent)`
        );
      } else {
      ...
      if (selectedLens.promptModifiers.length > 0) {
        const relevantMods = selectedLens.promptModifiers.filter(
          mod =>
      ...
      if (dofSettings.sensorSize !== 'full-frame') {
        parts.push(
          `${dofSettings.focalLength}mm lens (${dofResult.equivalentFocalLength}mm equivalent)`
        );
      } else {
    ...
    if (dofResult.totalDOF !== Infinity) {
      parts.push(
        `sharp from ${dofResult.nearFocus.toFixed(1)}m to ${dofResult.farFocus.toFixed(1)}m`
      );
    } else {
  ...
  const toggleGuide = (guideId: string) => {
    setFramingGuides(prev => prev.map(g => (g.id === guideId ? { ...g, enabled: !g.enabled } : g)));
  };
  ...
  const handleViewportClick = useCallback(
    (e: React.MouseEvent<HTMLDivElement>) => {
      ...
      console.log(
        `[Viewfinder] Click-to-focus: (${x.toFixed(2)}, ${y.toFixed(2)}) → ${estimatedDistanceM.toFixed(1)}m${snapResult.snapped ? ' [SNAPPED]' : ''}`
      );
    },
    ...
    const drawTechnicalStrip = () => {
      ...
      const technicalData = [
        ...
        { label: 'FOCUS', value: `${recipe.focusDistanceM.toFixed(1)}m`, color: '#4ade80' }, // green
        {
          label: 'DOF',
          value:
            recipe.depthOfField.totalM === Infinity
              ? '∞'
              : `${recipe.depthOfField.totalM.toFixed(1)}m`,
          color: '#ffffff',
        },
        {
          label: 'SENSOR',
          value:
            dofSettings.sensorSize === 'full-frame'
              ? 'FF'
              : dofSettings.sensorSize === 'aps-c'
                ? 'APS-C'
                : 'M4/3',
          color: '#a78bfa',
        }, // purple
  ...
  const panelContent = (
    <div className="flex h-full bg-[#0a0a0a]">
      ...
      <div className="flex flex-1 flex-col">
        ...
        <div className="flex items-center justify-between border-b border-white/10 px-3 py-2">
          <div className="flex items-center gap-2">
            ...
            {onTogglePageSidebar && (
              <Tooltip
                content={pageSidebarCollapsed ? 'Show References' : 'Hide References'}
                side="bottom"
              >
                <button
        ...
        <div className="relative flex-1 overflow-hidden bg-black">
          ...
          >
                  ...
                  >
                    ...
                    {arActive && activeTab === 'ar' ? (
                      ...
                      <img
                        ...
                        style={getLookFilterStyle(selectedCameraSpec)}
                      />
                    ) : (
                ...
                <>
                  ...
                  {focusPoint && (
                    ...
                    >
                      ...
                      <div className="absolute top-0 left-6 rounded bg-black/80 px-2 py-0.5 text-[10px] font-medium whitespace-nowrap text-green-400">
                        {dofResult.focusDistanceM.toFixed(1)}m
                      </div>
                      {/* DOF range label */}
                      <div className="absolute top-5 left-6 rounded bg-black/60 px-2 py-0.5 text-[9px] whitespace-nowrap text-gray-400">
                        {dofResult.nearFocus.toFixed(1)}m -{' '}
                        {dofResult.farFocus === Infinity
                          ? '∞'
                          : `${dofResult.farFocus.toFixed(1)}m`}
                      </div>
                    </div>
                  )}
            ...
            {/* End of Global Camera Movement Transform Layer */}
          </div>
              ...
              <div className="flex items-center gap-3 rounded-lg bg-black/60 px-3 py-1">
                ...
                <div className="text-center">
                  ...
                  <div className="font-mono text-xs text-amber-400">
                    {dofSettings.focalLength}mm
                  </div>
                </div>
                {/* 2D Flat Composite note - manages user expectations */}
                <div className="h-4 w-px bg-white/10" />
                <Tooltip
                  content="This is a 2D CSS simulation. Real 3D perspective transforms would require WebGL or Three.js."
                  side="top"
                >
                  <div className="flex items-center gap-1 text-gray-500">
                    <Info className="h-3 w-3" />
                    <span className="text-[8px] tracking-wider uppercase">2D Sim</span>
                  </div>
                </Tooltip>
              </div>
            )}
              ...
              {activeTab === 'dof' && (
                <>
                    ...
                    {dofResult.isDiffractionLimited && (
                      <div className="mt-2 flex items-center gap-1.5 rounded bg-amber-500/10 px-2 py-1">
                        ...
                        <span className="text-[9px] text-amber-300">
                          Diffraction at f/{dofSettings.aperture}
                        </span>
                      </div>
                    )}
                  </div>
                  ...
                  <AccordionSection
                    ...
                    icon={Aperture}
                    color="cyan"
                    isExpanded={expandedSections.has('aperture')}
                    ...
                    <div className="space-y-3">
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={e =>
                            setDofSettings(prev => ({
                              ...prev,
                              aperture: APERTURE_STOPS[Number(e.target.value)],
                            }))
                          }
                        ...
                        <p className="mt-1 text-[8px] leading-relaxed text-gray-600">
                          CSS blur simulation. Final generation uses{' '}
                          <span className="text-cyan-400">Learn2Refocus</span> for photorealistic
                          bokeh.
                        </p>
                      </div>
                      {/* Focus Distance */}
                      <div>
                        ...
                        <input
                          ...
                          onChange={e =>
                            setDofSettings(prev => ({
                              ...prev,
                              focusDistance: Number(e.target.value) / 100,
                            }))
                          }
                      ...
                      <div className="flex h-1.5 overflow-hidden rounded-full bg-gray-800">
                        <div
                          ...
                          style={{ width: `${dofResult.frontPercent}%` }}
                        />
                        <div className="w-px bg-white" />
                        <div
                          className="bg-purple-500"
                          style={{ width: `${dofResult.backPercent}%` }}
                        />
                      </div>
                      <div className="flex justify-between text-[8px] text-gray-600">
                        <span>{dofResult.frontDOF.toFixed(1)}m front</span>
                        <span>
                          {dofResult.backDOF === Infinity
                            ? '∞'
                            : `${dofResult.backDOF.toFixed(1)}m`}{' '}
                          back
                        </span>
                      </div>
                    </div>
                  ...
                  <AccordionSection
                    ...
                    icon={Target}
                    color="green"
                    isExpanded={expandedSections.has('focus-controls')}
                    onToggle={toggleSection}
                  >
                    <div className="space-y-3">
                  ...
                  <AccordionSection
                    ...
                    icon={Move}
                    color="cyan"
                    isExpanded={expandedSections.has('motion')}
                    onToggle={toggleSection}
                  >
                    <div className="space-y-3">
                  ...
                  <AccordionSection
                    ...
                    icon={Layers}
                    color="purple"
                    isExpanded={expandedSections.has('scene')}
                    onToggle={toggleSection}
                  >
                    <div className="space-y-3">
                      <div className="flex items-center justify-between">
                          ...
                          <div>
                            <div className="mb-1 flex justify-between text-[9px]">
                              ...
                              <span className="text-white">
                                {backgroundDistance >= 100
                                  ? '∞'
                                  : `${backgroundDistance.toFixed(0)}m`}
                              </span>
                            </div>
                            <input
                              type="range"
                  ...
                  <AccordionSection
                    ...
                    icon={Circle}
                    color="amber"
                    isExpanded={expandedSections.has('bokeh')}
                    onToggle={toggleSection}
                  >
                    <div className="space-y-2">
                      <div className="flex flex-wrap gap-1">
                        {(Object.keys(BOKEH_PRESETS) as Array<keyof typeof BOKEH_PRESETS>).map(
                          preset => (
                            <button
                              key={preset}
                            ...
                            >
                              <svg width="10" height="10" viewBox="0 0 24 24">
                                <path
                                  d={generateBokehPath(
                                    BOKEH_PRESETS[preset].bladeCount,
                                    24,
                                    BOKEH_PRESETS[preset].rotation,
                                    BOKEH_PRESETS[preset].curvature
                                  )}
                                  fill="currentColor"
                                />
                              </svg>
                      ...
                      <div className="text-[8px] text-gray-600">
                        {bokehSettings.bladeCount} blades
                      </div>
                    </div>
                  </AccordionSection>
                    ...
                    <div className="space-y-2">
                      <div className="flex flex-wrap gap-1">
                        {BUILT_IN_DOF_PRESETS.map(preset => (
                          <Tooltip
                            key={preset.id}
                            content={preset.description || preset.name}
                            side="left"
                          >
                            <button
                        ...
                        <div className="max-h-20 space-y-1 overflow-y-auto">
                          {userPresets.map(preset => (
                            ...
                            >
                              <button
                                onClick={() => applyPreset(preset)}
                                className="text-[9px] text-white hover:text-green-300"
                              >
                                {preset.name}
                              </button>
                              <button
                      ...
                      <div className="flex gap-1">
                        <button
                          ...
                          className="flex-1 rounded border border-white/10 bg-white/5 py-1 text-[8px] text-gray-400 hover:text-cyan-300"
                        >
                          Export
                        </button>
                  ...
                  <AccordionSection
                    ...
                    icon={User}
                    color="cyan"
                    isExpanded={expandedSections.has('framing')}
                    onToggle={toggleSection}
                  >
                    <div className="space-y-3">
                      ...
                      {selectedFraming && (
                        <div className="rounded bg-cyan-500/10 px-2 py-1.5 text-[9px] text-cyan-300">
                          ...
                          <div className="mt-1 text-[8px] text-gray-500">
                            Subject @ {selectedFraming.modelDistance}m • Scale:{' '}
                            {selectedFraming.modelScale}×
                          </div>
                        </div>
                      )}
                    </div>
                  ...
                  <AccordionSection
                    ...
                    icon={Scissors}
                    color="purple"
                    isExpanded={expandedSections.has('layers-ai')}
                    onToggle={toggleSection}
                  >
                    <div className="space-y-3">
                      ...
                      <div className="rounded border border-white/10 bg-white/5 p-2">
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          ...
                          >
                            {extractedLayers?.subject ? (
                              <img
                                src={extractedLayers.subject}
                                alt="Subject"
                                className="mx-auto h-12 w-full rounded object-cover"
                              />
                            ) : (
                          ...
                          >
                            {extractedLayers?.background ? (
                              <img
                                src={extractedLayers.background}
                                alt="Background"
                                className="mx-auto h-12 w-full rounded object-cover"
                              />
                            ) : (
                        ...
                        >
                          {extractedLayers?.foreground ? (
                            <img
                              src={extractedLayers.foreground}
                              alt="Foreground"
                              className="mx-auto h-10 w-full rounded object-cover"
                            />
                          ) : (
                            <div className="flex items-center justify-center gap-1">
                              <Upload className="h-3 w-3 text-gray-500 group-hover:text-amber-400" />
                              <span className="text-[8px] text-gray-500 group-hover:text-amber-400">
                                + Foreground (optional)
                              </span>
                            </div>
                          )}
                    ...
                    >
                      <SceneDepthControls
                        ...
                        onFocusDistanceChange={distM => {
                          ...
                          setDofSettings(prev => ({
                            ...prev,
                            focusDistance: Math.max(0, Math.min(1, normalized)),
                          }));
                        }}
                        isExpanded={expandedSections.has('scene-depth')}
                      />
                  ...
                  >
                    <DollyZoomSimulator
                      ...
                      onDistanceChange={distM => {
                        ...
                        const matching = FRAMING_PRESETS.find(
                          f => Math.abs(f.modelDistance - distM) < 0.5
                        );
                        if (matching) {
                      ...
                      <div>
                        <div className="mb-1 flex items-center justify-between">
                          ...
                          <span className="font-mono text-xs text-cyan-400">
                            {Math.round(
                              (localElements.find(el => el.id === selectedElementId)?.scale || 1) *
                                100
                            )}
                            %
                          </span>
                      ...
                      <div>
                        <div className="mb-1 flex items-center justify-between">
                          ...
                          <span className="font-mono text-xs text-amber-400">
                            {Math.round(
                              (localElements.find(el => el.id === selectedElementId)?.opacity ||
                                1) * 100
                            )}
                            %
                          </span>
                        ...
                        <input
                          ...
                          max={100}
                          value={
                            (localElements.find(el => el.id === selectedElementId)?.opacity || 1) *
                            100
                          }
                          onChange={e =>
                            handleElementUpdate(selectedElementId, {
      ...
      <CameraLensSelector
        ...
        selectedCamera={selectedCameraSpec}
        selectedLensFamily={selectedLensFamily}
        selectedFocalLength={selectedFocalLength}
        onApply={(newCamera, newLensFamily, newFocalLength) => {
          setSelectedCameraSpec(newCamera);
          setSelectedLensFamily(newLensFamily);
          setSelectedFocalLength(newFocalLength);
          // Update DOF settings based on new camera/lens selection
          if (newFocalLength) {
            setDofSettings(prev => ({ ...prev, focalLength: newFocalLength }));
          }
          // Update sensor size based on camera spec
          if (newCamera?.sensor_spec) {
            const cropFactor = newCamera.sensor_spec.crop_factor_ff;
            if (cropFactor <= 1.1) {
              setDofSettings(prev => ({ ...prev, sensorSize: 'full-frame' }));
            } else if (cropFactor <= 1.7) {
              setDofSettings(prev => ({ ...prev, sensorSize: 'aps-c' }));
            } else {
              setDofSettings(prev => ({ ...prev, sensorSize: 'micro-four-thirds' }));
            }
          }
        }}
      />
  ...
  if (embedded) {
    ...
    if (fullscreen) {
      return <div className="h-full w-full bg-[#0a0a0a]">{panelContent}</div>;
    }
