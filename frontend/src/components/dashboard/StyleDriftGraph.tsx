...
export function StyleDriftGraph({ projectId, days = 30, onDaysChange }: StyleDriftGraphProps) {
  ...
  const trend = useMemo((): { direction: 'up' | 'down' | 'stable'; change: number } => {
    ...
    const avgFirst = Object.keys(firstHalf[0] || {}).reduce(
      (acc, key) => {
    ...
    const avgSecond = Object.keys(secondHalf[0] || {}).reduce(
      (acc, key) => {
      ...
      <div className="mb-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          >
            ...
            <span>
              {trend.direction === 'stable' ? 'Stable' : `${Math.abs(trend.change).toFixed(1)}%`}
            </span>
          </div>
            ...
            {[7, 14, 30].map(d => (
              <button
                ...
                className={`rounded px-2 py-1 text-xs transition-colors ${
                  days === d ? 'bg-purple-500 text-white' : 'text-white/50 hover:text-white'
                }`}
              >
                {d}d
              </button>
      ...
      <div className="relative">
        ...
        {hoveredPoint !== null && data[hoveredPoint] && (
          <motion.div
            ...
            className="pointer-events-none absolute top-0 left-1/2 z-10 -translate-x-1/2 rounded-lg border border-white/10 bg-zinc-800 p-3 shadow-xl"
          >
            ...
            <div className="grid grid-cols-2 gap-x-4 gap-y-1">
              {Object.entries(data[hoveredPoint][activeCategory]).map(([key, value]) => (
                <div key={key} className="flex items-center gap-2 text-xs">
                  ...
                  <span className="font-medium text-white">{(value as number).toFixed(1)}%</span>
                </div>
              ))}
            </div>
          </motion.div>
