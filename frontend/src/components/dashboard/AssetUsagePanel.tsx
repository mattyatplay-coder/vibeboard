  ...
  if (error) {
    return (
      <div className="rounded-xl border border-red-500/30 bg-red-500/10 p-6">
        ...
        <button onClick={fetchData} className="mt-2 text-sm text-red-300 hover:text-red-200">
          Retry
        </button>
      </div>
    );
          ...
          {filteredAssets.length === 0 ? (
            ...
            filteredAssets.map((asset, index) => (
              ...
              >
                ...
                <button
                  onClick={() =>
                    setExpandedAsset(expandedAsset === asset.assetId ? null : asset.assetId)
                  }
                  className="flex w-full items-center gap-3 p-3 text-left transition-colors hover:bg-white/5"
                >
                  {/* Thumbnail or icon */}
                  <div
                    className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-lg ${ASSET_COLORS[asset.assetType]}`}
                  >
                    {asset.thumbnail ? (
                      <Image
                        src={asset.thumbnail}
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {asset.triggerWord && (
                      <p className="truncate text-xs text-purple-400">{asset.triggerWord}</p>
                    )}
                  </div>
                    ...
                    >
                      <div className="p-3">
                          ...
                          {asset.byDay.slice(-14).map((day, i) => {
                            ...
                            return (
                              <div key={day.date} className="group relative flex-1">
                                ...
                                />
                                <div className="absolute bottom-full left-1/2 mb-1 hidden -translate-x-1/2 rounded bg-zinc-700 px-2 py-1 text-xs whitespace-nowrap text-white group-hover:block">
                                  {day.count} on{' '}
                        ...
                        <p className="mt-2 text-xs text-white/30">
                          Last used:{' '}
                          {new Date(asset.lastUsed).toLocaleDateString('en-US', {
                            month: 'short',
                            day: 'numeric',
                            hour: 'numeric',
                            minute: '2-digit',
