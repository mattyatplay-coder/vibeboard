...
export function RenderQueuePanel({
    ...
    const fetchJobStatus = async (jobId: string) => {
        try {
            const res = await fetch(`${BACKEND_URL}/api/projects/${projectId}/render-queue/jobs/${jobId}`);
            if (res.ok) {
    ...
    const handleCancel = async () => {
        ...
        try {
            await fetch(
                `${BACKEND_URL}/api/projects/${projectId}/render-queue/jobs/${currentJob.id}`,
                { method: 'DELETE' }
            );
            setCurrentJob(null);
        } catch (error) {
    ...
    const handleAcceptMaster = useCallback(async (shotId: string, passId: string) => {
        try {
    ...
    const handleRejectMaster = useCallback(async (shotId: string) => {
        try {
            const res = await fetch(
                `${BACKEND_URL}/api/projects/${projectId}/render-queue/scene-chains/${sceneChainId}/shots/${shotId}/finalize`,
                {
    ...
    useEffect(() => {
        const shotsNeedingReview = versionStacks.filter(stack => {
            const hasMaster = stack.versions.some(v => v.quality === 'master' && v.status === 'complete');
                    ...
                    >
                        <div className="px-4 pb-4 space-y-4">
                            ...
                            {selectedQualities.length > 0 && !currentJob && (
                                <div className="flex items-center justify-between text-xs text-gray-400 bg-white/5 rounded-lg px-3 py-2">
                                    ...
                                    <span className="text-gray-500">
                                        {shotCount} shot{shotCount !== 1 ? 's' : ''} × {selectedQualities.length} tier{selectedQualities.length !== 1 ? 's' : ''}
                                    </span>
                                </div>
                            )}
                                                ...
                                                {currentJob.passes
                                                    ...
                                                    .map(p => (
                                                        <button
                                                            ...
                                                            className="text-[10px] px-2 py-1 rounded bg-red-500/20 text-red-400 hover:bg-red-500/30 flex items-center gap-1"
                                                        >
                                ...
                                {!currentJob ? (
                                    ...
                                    <>
                                        ...
                                        <button
                                            ...
                                            className="px-4 py-2.5 rounded-lg text-sm font-medium bg-red-500/20 text-red-400 hover:bg-red-500/30 transition-all"
                                        >
                                            Cancel
                                        </button>
                                    </>
                            ...
                            {costComparison && !currentJob && (
                                <div className="text-[10px] text-gray-500 bg-white/5 rounded-lg px-3 py-2">
                                    <div className="flex items-center gap-1 text-green-400 mb-1">
                                        <TrendingDown className="w-3 h-3" />
                                        <span className="font-medium">Draft-First Workflow Saves Money</span>
                                    </div>
                                    <p>
                                        Iterate 3× at draft quality (${costComparison.draftCost.toFixed(2)}),
                                        then render once at master (${costComparison.masterCost.toFixed(2)}).
                                        Save ${costComparison.savings.toFixed(2)} vs. iterating at master quality.
                                    </p>
                                </div>
                            )}
                                ...
                                <div className="space-y-2">
                                    <div className="flex items-center justify-between">
                                        <div className="flex items-center gap-2 text-[10px] text-gray-500 uppercase tracking-wider">
                                            ...
                                            {pendingReviewShots.size > 0 && (
                                                <span className="ml-1 px-1.5 py-0.5 rounded-full bg-orange-500/20 text-orange-400 text-[9px] font-bold">
                                        ...
                                        <div className="flex items-center gap-2">
                                            ...
                                            {pendingReviewShots.size > 0 && (
                                                <button
                                                    onClick={() => {
                                                        const firstPendingShot = versionStacks.find(
                                                            s => pendingReviewShots.has(s.shotId)
                                                        );
                                                        setLightboxShotId(firstPendingShot?.shotId);
                                                        setShowLightbox(true);
                                                    }}
                                                    className="flex items-center gap-1.5 px-2.5 py-1.5 rounded text-[10px] font-medium bg-gradient-to-r from-orange-500/30 to-amber-500/30 text-orange-300 hover:from-orange-500/40 hover:to-amber-500/40 border border-orange-500/30 transition-all"
                                                >
                                            ...
                                            {versionStacks.some(s => s.versions.filter(v => v.status === 'complete').length >= 2) && pendingReviewShots.size === 0 && (
                                                <button
                                                    onClick={() => {
                                                    ...
                                                    className="flex items-center gap-1.5 px-2 py-1 rounded text-[10px] font-medium bg-cyan-500/20 text-cyan-400 hover:bg-cyan-500/30 transition-all"
                                                >
                                    ...
                                    <div className="space-y-1.5 max-h-48 overflow-y-auto">
                                        {versionStacks.map((stack) => {
                                            const hasMultiplePasses = stack.versions.filter(v => v.status === 'complete').length >= 2;
                                            ...
                                            return (
                                                <div
                                                    ...
                                                    className={clsx(
                                                        ...
                                                        isPendingReview && "bg-orange-500/10 border border-orange-500/20",
                                                ...
                                                >
                                                    <div className="flex items-center gap-3">
                                                        <div className="flex items-center gap-2">
                                                            ...
                                                            {isApproved && (
                                                                <span className="px-1.5 py-0.5 rounded text-[9px] font-medium bg-green-500/20 text-green-400 flex items-center gap-1">
                                                            ...
                                                            {(['draft', 'review', 'master'] as RenderQuality[]).map((q) => {
                                                                ...
                                                                if (!version) {
                                                                    return (
                                                                        <div
                                                                            key={q}
                                                                            className="w-6 h-6 rounded border border-white/10 border-dashed"
                                                                ...
                                                                return (
                                                                    <div
                                                                        ...
                                                                        className={clsx(
                                                                            ...
                                                                            version.status === 'generating' && "border-2 border-blue-400 animate-pulse",
                                                                            version.status === 'failed' && "bg-red-500/20 border border-red-500/30 text-red-400",
                                                                            version.status === 'pending' && "border border-white/20 text-gray-500",
                                                                            // Highlight master badge when pending review
                                                                            q === 'master' && isPendingReview && version.status === 'complete' && "ring-2 ring-orange-400/50 ring-offset-1 ring-offset-black"
                                                                        )}
                                                                        title={`${q} - ${version.status}${version.seed ? ` (seed: ${version.seed})` : ''}${isPendingReview && q === 'master' ? ' - PENDING REVIEW' : ''}`}
                                                                    >
                                                                        {version.status === 'complete' && (q === 'draft' ? 'D' : q === 'review' ? 'R' : 'M')}
                                                                        {version.status === 'generating' && <Loader2 className="w-3 h-3 animate-spin" />}
                                                    ...
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        {isPendingReview && (
                                                            <button
                                                                ...
                                                                className="flex items-center gap-1.5 px-2 py-1 rounded text-[10px] font-medium bg-orange-500/20 text-orange-400 hover:bg-orange-500/30 border border-orange-500/30 transition-all"
                                                            >
                                                        ...
                                                        {!isPendingReview && hasMultiplePasses && (
                                                            <button
                                                                ...
                                                                className="p-1 rounded text-cyan-400 hover:bg-cyan-500/20 transition-all"
                                                        ...
                                                        {stack.canUpgrade && currentJob && !isPendingReview && !isApproved && (
                                                            <button
                                                                onClick={() => handlePromoteShot(stack.shotId, stack.nextUpgradeQuality!)}
                                                                disabled={promotingShot === stack.shotId}
                                                                className={clsx(
                                                                    "flex items-center gap-1 px-2 py-1 rounded text-[10px] font-medium transition-all",
                                                                    QUALITY_COLORS[stack.nextUpgradeQuality!],
                                                                    "hover:scale-105",
                                                                    promotingShot === stack.shotId && "opacity-50 cursor-wait"
                                                                )}
                                                            >
                                                                ...
                                                                {stack.upgradeCost !== undefined && (
                                                                    <span className="opacity-70">(${stack.upgradeCost.toFixed(2)})</span>
                                                                )}
                                                            </button>
                                                        )}
                                                    </div>
