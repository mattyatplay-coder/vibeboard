...
export const useProducerAgent = (variations: number = 1) => {
  const { currentModelId, currentDuration, currentAspectRatio, isVideo, setCurrentConfig } =
  ...
  const handleAction = useCallback(
    (alertId: string, actionType?: string, actionValue?: string) => {
    ...
    if (aspectRatio === '16:9' && modelId.toLowerCase().includes('anamorphic')) {
      ...
      if (!acknowledgedAlerts.has(alertId)) {
        newAlerts.push({
          ...
          message:
            'Anamorphic lens selected with 16:9 aspect ratio. Consider switching to 21:9 for authentic cinematic look.',
          severity: 'Technical',
          actionType: 'hide_warning',
        });
      }
