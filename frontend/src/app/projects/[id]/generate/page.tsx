...
export default function GeneratePage() {
    ...
    const loadGenerations = async () => {
        try {
            ...
            // Track costs for newly succeeded generations
            // Compare with previous state to find generations that just succeeded
            const prevGenIds = new Set(generations.filter(g => g.status === 'succeeded').map(g => g.id));
            const newlySucceeded = data.filter((g: Generation) =>
                g.status === 'succeeded' && !prevGenIds.has(g.id)
            );

            for (const gen of newlySucceeded) {
                const modelId = gen.falModel || gen.engine;
                const model = ALL_MODELS.find(m => m.id === modelId);
                if (model && modelId) {
                    // Parse duration from generation metadata if available
                    const durationMatch = gen.usedLoras?.duration?.match(/(\d+)/);
                    ...
                    costTracker.recordGeneration({
                        ...
                        durationSeconds: model.type === 'video' ? durationSeconds : undefined,
                    });
                }
            }
