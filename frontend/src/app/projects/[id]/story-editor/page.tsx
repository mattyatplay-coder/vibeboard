    ...
    const runPipeline = async () => {
        ...
        try {
            ...
            for (let i = 0; i < parseResponse.scenes.length; i++) {
                ...
                const breakdownResponse = await fetchAPI('/story-editor/breakdown', {
                    ...
                    body: JSON.stringify({
                        ...
                        genre: selectedGenre,
                        config: { pace, style, targetDuration: targetDurationSeconds, totalScenes: parseResponse.scenes.length, allowNSFW }
                    })
                });
            ...
            for (let i = 0; i < breakdowns.length; i++) {
                ...
                if (shots.length === 0) {
                    console.warn(`Scene ${i + 1} has no suggestedShots array, checking for alternate keys...`);
                                    ...
                                    <div key={i} className="bg-black/30 rounded-lg p-4">
                                        <div className="flex items-center justify-between mb-2">
                                            <h4 className="font-bold text-white">
                                                Scene {scene.sceneNumber}: {scene.heading?.intExt}. {scene.heading?.location}
                                            </h4>
                                            <span className={clsx(
                                                "px-2 py-0.5 rounded text-[10px] font-bold uppercase",
                            ...
                            <div className="space-y-3">
                                {prompts.map((prompt, i) => (
                                    <div key={i} className="bg-black/30 rounded-lg p-4">
                                        ...
                                        <p className="text-xs text-green-300 bg-green-500/10 rounded p-2 mb-2">
                                            {prompt.prompt}
                                        </p>
