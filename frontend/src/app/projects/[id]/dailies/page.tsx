...
export default function DailiesPage() {
  ...
  const fetchComments = useCallback(async () => {
    try {
      setIsLoading(true);
      const resolvedParam =
        filter === 'all' ? '' : filter === 'unresolved' ? '&resolved=false' : '';
      const res = await fetch(
        `${apiUrl}/api/projects/${projectId}/comments?limit=100${resolvedParam}`
      );
      const data = await res.json();
      if (data.success) {
  ...
  const getMediaInfo = (gen: Generation | null): { url: string | null; isVideo: boolean } => {
    ...
    try {
      ...
      if (Array.isArray(parsed) && parsed.length > 0) {
        ...
        const isVideo =
          output.type === 'video' ||
          (url?.includes('.mp4') ?? false) ||
          (url?.includes('/video/') ?? false);
        return { url, isVideo };
      }
    } catch {
  ...
  const generationsWithComments = Array.from(
    new Map(comments.filter(c => c.generation).map(c => [c.generationId, c.generation])).values()
  ) as Generation[];
    ...
    <div className="flex h-screen bg-[#0a0a0a]">
      ...
      <main className="flex flex-1 overflow-hidden">
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="p-3 text-left">
                        <p className="line-clamp-2 text-xs text-gray-300">{gen.inputPrompt}</p>
                        {genComments.length > 0 ? (
                          <div className="mt-2 flex items-center gap-2 text-xs text-gray-500">
                            <span>{unresolvedCount} open</span>
                            <span>•</span>
                            <span>{genComments.filter(c => c.resolved).length} resolved</span>
                          </div>
                        ) : (
                          <div className="mt-2 text-xs text-gray-600">No comments</div>
                        )}
                      </div>
                    </button>
                  );
        ...
        <div className="w-80 flex-shrink-0 border-l border-white/10 bg-zinc-900/50">
          <div className="border-b border-white/10 p-4">
            <h2 className="text-sm font-medium text-white">Comments ({currentComments.length})</h2>
          </div>
          ...
          <div className="overflow-y-auto p-4" style={{ height: 'calc(100vh - 60px)' }}>
            {currentComments.length === 0 ? (
              <div className="py-12 text-center">
                ...
                <p className="mt-2 text-xs text-gray-600">Click "Add Annotation" to start</p>
              </div>
            ) : (
