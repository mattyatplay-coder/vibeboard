...
const createTestApp = () => {
  ...
  app.get('/api/projects/:projectId/generations/:id', (req, res) => {
    const generation = generations.find(
      g => g.id === req.params.id && g.projectId === req.params.projectId
    );
    if (!generation) {
...
describe('Generations API', () => {
  ...
  describe('GET /api/projects/:projectId/generations', () => {
    ...
    it('should return generations with correct structure', async () => {
      const response = await request(app).get('/api/projects/proj-1/generations').expect(200);
  ...
  describe('GET /api/projects/:projectId/generations/:id', () => {
    it('should return a specific generation', async () => {
      const response = await request(app).get('/api/projects/proj-1/generations/gen-1').expect(200);
    ...
    it('should return 404 for non-existent generation', async () => {
      await request(app).get('/api/projects/proj-1/generations/non-existent').expect(404);
    });
  });
  ...
  describe('DELETE /api/projects/:projectId/generations/:id', () => {
    it('should delete a generation', async () => {
      await request(app).delete('/api/projects/proj-1/generations/gen-1').expect(204);

      await request(app).get('/api/projects/proj-1/generations/gen-1').expect(404);
    });
  });
  ...
  describe('POST /api/projects/:projectId/generations/:id/retry', () => {
    ...
    it('should return 404 for non-existent generation', async () => {
      await request(app).post('/api/projects/proj-1/generations/non-existent/retry').expect(404);
    });
  });
});
