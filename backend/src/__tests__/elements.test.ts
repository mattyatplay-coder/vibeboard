...
const createTestApp = () => {
    ...
    app.get('/api/projects/:projectId/elements/:id', (req, res) => {
        const element = elements.find(e => e.id === req.params.id && e.projectId === req.params.projectId);
        if (!element) {
    ...
    app.put('/api/projects/:projectId/elements/:id', (req, res) => {
        const index = elements.findIndex(e => e.id === req.params.id && e.projectId === req.params.projectId);
        if (index === -1) {
            return res.status(404).json({ error: 'Element not found' });
        }
        elements[index] = {
...
describe('Elements API', () => {
    ...
    describe('GET /api/projects/:projectId/elements', () => {
        ...
        it('should return empty array for project with no elements', async () => {
            const response = await request(app)
                .get('/api/projects/proj-999/elements')
                .expect(200);
    ...
    describe('GET /api/projects/:projectId/elements/:id', () => {
        it('should return a specific element', async () => {
            const response = await request(app)
                .get('/api/projects/proj-1/elements/elem-1')
                .expect(200);
        ...
        it('should return 404 for non-existent element', async () => {
            await request(app)
                .get('/api/projects/proj-1/elements/non-existent')
                .expect(404);
        });
    });
    ...
    describe('DELETE /api/projects/:projectId/elements/:id', () => {
        it('should delete an element', async () => {
            await request(app)
                .delete('/api/projects/proj-1/elements/elem-1')
                .expect(204);
