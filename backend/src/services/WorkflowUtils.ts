...
export function applyWorkflowMapping(
    ...
    if (map.negative && workflow[map.negative.nodeId]) {
        workflow[map.negative.nodeId].inputs[map.negative.inputId || 'text'] = options.negativePrompt || "";
    }
    if (map.seed && workflow[map.seed.nodeId]) {
        workflow[map.seed.nodeId].inputs[map.seed.inputId || 'seed'] = options.seed || Math.floor(Math.random() * 1000000000);
    }
