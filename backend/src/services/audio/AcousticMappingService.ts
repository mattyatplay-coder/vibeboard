...
export const MOOD_ACOUSTIC_MAPPINGS: MoodAcousticMapping[] = [
    {
        mood: 'noir',
        keywords: ['dark brooding low-frequency drone', 'moody silence', 'tense undertones', 'shadowy ambiance'],
...
export const GENRE_IR_RECOMMENDATIONS: GenreIRRecommendation[] = [
    { genre: 'sci-fi', irType: 'metallic_hall', description: 'Metallic/Industrial reverb', reverbCharacter: 'cold, reflective, spaceship corridors' },
    { genre: 'western', irType: 'canyon', description: 'Open Canyon reverb', reverbCharacter: 'vast, natural, desert echo' },
    { genre: 'horror', irType: 'basement', description: 'Dark Basement reverb', reverbCharacter: 'claustrophobic, damp, unsettling' },
    { genre: 'fantasy', irType: 'cathedral', description: 'Stone Cathedral reverb', reverbCharacter: 'majestic, ancient, mystical' },
    { genre: 'noir', irType: 'jazz_club', description: 'Smoky Jazz Club reverb', reverbCharacter: 'intimate, warm, vintage' },
    { genre: 'thriller', irType: 'parking_garage', description: 'Concrete Parking Garage', reverbCharacter: 'cold, urban, echoey' },
    { genre: 'romance', irType: 'living_room', description: 'Warm Living Room', reverbCharacter: 'cozy, intimate, natural' },
    { genre: 'action', irType: 'stadium', description: 'Large Stadium/Arena', reverbCharacter: 'massive, impactful, energetic' },
];
...
export class AcousticMappingService {
    ...
    private analyzeMood(recipe: ShotRecipe): { moodPrompt: string; isNoir: boolean } {
        ...
        const isNoir = moodTag.includes('noir') ||
        ...
        const moodMapping = MOOD_ACOUSTIC_MAPPINGS.find(m =>
            moodTag.includes(m.mood) || lightingPrompt.includes(m.mood)
        );
    ...
    private extractAudioContext(prompt: string): string {
        ...
        if (lowerPrompt.includes('ocean') || lowerPrompt.includes('beach') || lowerPrompt.includes('water')) {
        ...
        if (lowerPrompt.includes('fight') || lowerPrompt.includes('battle') || lowerPrompt.includes('combat')) {
    ...
    getGenreIRRecommendation(genre: string): GenreIRRecommendation | undefined {
        return GENRE_IR_RECOMMENDATIONS.find(ir =>
            ir.genre.toLowerCase() === genre.toLowerCase()
        );
    }
     ...
     */
    getInterpolatedValues(focalLength: number): { reverb: number; stereoWidth: number; foleyDetail: number } {
        ...
        return {
            ...
            stereoWidth: lowerMapping.stereoWidth + (upperMapping.stereoWidth - lowerMapping.stereoWidth) * t,
            foleyDetail: lowerMapping.foleyDetail + (upperMapping.foleyDetail - lowerMapping.foleyDetail) * t
        };
    }
}
