...
export class AnalysisService {
  ...
  public async analyzeGeneration(
    generationId: string,
    userFeedback?: string
    ...
    const isVideo =
      ...
      (generation.outputs && generation.outputs.includes('"type":"video"'));

    let targetImages: string[] = [imageUrl];

    if (isVideo) {
      console.log(
        `[AnalysisService] Detected video. Extracting multiple frames for temporal analysis...`
      );
      try {
    ...
    if (generation && (generation as any).sourceImages) {
      try {
        ...
        if (Array.isArray(sources) && sources.length > 0) {
          console.log(
            `[AnalysisService] Found ${sources.length} source images. Adding to analysis.`
          );
    ...
    try {
      if (generation.sourceElementIds) {
        ...
        elements.forEach(element => {
          ...
          else if (type.includes('pose') || type.includes('composition'))
            semanticLabel = 'REFERENCE (Pose/Structure)';
    ...
    for (const fail of failures) {
      ...
      if (loraMatch || keywordMatch) {
        try {
          ...
          if (analysis.advice) {
            distinctLessons.add(
              `Past failure (${fail.rating}/5 stars): ${analysis.advice}. Issue was: ${analysis.flaws[0]}`
            );
          }
        } catch (e) {}
      }
