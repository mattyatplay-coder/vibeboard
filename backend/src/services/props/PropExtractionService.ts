...
export class PropExtractionService {
     ...
     */
    async batchExtract(
        sources: Array<{ url: string; type: 'generation' | 'upload' | 'element' | 'url'; name?: string }>,
        options: ExtractionOptions
        ...
        console.log(`[PropExtraction] Batch complete: ${results.successful.length} succeeded, ${results.failed.length} failed`);
        return results;
    }
    ...
    private async runBiRefNet(imageUrl: string, model: string): Promise<string> {
        const endpoint = model === 'birefnet-massive'
            ? 'fal-ai/birefnet/massive'
            : 'fal-ai/birefnet';
    ...
    private async analyzeMaterial(imagePath: string): Promise<MaterialAnalysis> {
        try {
            ...
            const avgBrightness = dominant.reduce((sum, c) => {
     ...
     */
    async updateProp(propId: string, data: {
        name?: string;
        category?: string;
        description?: string;
        isFavorite?: boolean;
