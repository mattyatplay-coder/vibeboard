...
export class FrameExtractor {
   ...
   */
  async extractBookendFrames(
    videoUrl: string
  ): Promise<{ first: ExtractedFrame; last: ExtractedFrame }> {
  ...
  async extractFramesByRate(videoUrl: string, fps: number = 1): Promise<ExtractedFrame[]> {
    try {
      ...
      console.log(
        `[FrameExtractor] Extracting ${timestamps.length} frames (Rate: ${fps}fps, Duration: ${duration}s)`
      );
  ...
  async extractFramesForRotoscope(
    ...
    try {
      ...
      console.log(
        `[FrameExtractor] Video: ${duration}s @ ${originalFps}fps, extracting ${framesToExtract} frames at ${targetFps}fps`
      );
  ...
  async reconstructVideo(
    ...
    await new Promise<void>((resolve, reject) => {
      ffmpeg()
        ...
        .outputOptions([`-c:v ${codec}`, '-pix_fmt yuv420p', '-crf 18'])
        .output(output)
        .on('end', () => resolve())
        .on('error', err => reject(err))
        .run();
