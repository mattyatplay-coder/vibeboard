...
export interface LoRAModel {
  ...
  tagDefinitions?: Record<string, string>; // Tag -> Description map
...
export class LoRARegistry {
  ...
  search(query: string, type?: LoRAModel['type']): LoRAModel[] {
    ...
    for (const lora of this.localRegistry.values()) {
      ...
      const triggerMatch = lora.triggerWords.some(t => t.toLowerCase().includes(lowerQuery));
  ...
  async fetchFromCivitai(modelId: string): Promise<LoRAModel | null> {
    ...
    try {
      ...
      const tagDefinitions = this.extractTagDefinitions(
  ...
  async searchCivitai(
    ...
    try {
      ...
      const response = await axios.get(`https://civitai.com/api/v1/models?${params}`, {
        timeout: 15000,
      });
  ...
  /**
   * Extract tag definitions (tag -> description)
   */
  private extractTagDefinitions(description: string): Record<string, string> {
    const tagDefs: Record<string, string> = {};
    if (!description) return tagDefs;
  ...
  private inferLoRAType(tags: string[], name: string): LoRAModel['type'] {
    ...
    if (lowerTags.includes('pose') || lowerTags.includes('action')) {
      return 'pose';
    }

    if (lowerTags.includes('background') || lowerTags.includes('environment')) {
      return 'background';
    }

    if (lowerTags.includes('effect') || lowerTags.includes('lighting')) {
      return 'effect';
    }
    ...
    try {
      ...
      for (const file of files) {
        if (file.match(/\.(safetensors|ckpt|pt)$/i)) {
          const lora = this.importFromPath(path.join(dirPath, file), options);
          imported.push(lora);
        }
      }
    } catch (error) {
