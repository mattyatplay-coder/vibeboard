...
export class DatasetService {
    ...
    async calculateReferenceEmbedding(filePaths: string[]): Promise<number[] | null> {
        ...
        console.log(`[DatasetService] Calculating reference embedding from ${filePaths.length} images...`);
    ...
    async curateDataset(
        ...
        if (!referenceEmbedding) {
            console.log(`[DatasetService] No reference embedding provided. Returning all ${candidatePaths.length} candidates (capped at ${limit}).`);
            return candidatePaths.slice(0, limit);
        }

        console.log(`[DatasetService] Curating ${candidatePaths.length} candidates against reference...`);
        ...
        for (let i = 0; i < candidatePaths.length; i += BATCH_SIZE) {
            ...
            await Promise.all(batch.map(async (filePath) => {
                try {
                    ...
                    if (bestFace && maxSim > 0.2) { // Minimum threshold to be considered same person at all
                        ...
                        console.log(`[DatasetService] Candidate ${debugName}: Match=${maxSim.toFixed(2)}, Score=${finalScore.toFixed(2)}`);
                    }
                } catch (e) {
        ...
        console.log(`[DatasetService] Selected top ${selected.length} from ${candidatePaths.length} candidates.`);
     ...
     */
    async processImage(filePath: string, datasetName: string, referenceEmbedding: number[] | null = null): Promise<ProcessingResult> {
        ...
        try {
            ...
            await sharp(processedBuffer as any)
