 ...
 */
export function parseDescriptionSettings(
  description: string | undefined
): ParsedRecommendedSettings {
  ...
  if (!settings.stepsRange) {
    settings.steps = findNumber([/steps[:\s]+(\d+)/i]);
  }
  ...
  // Clip Skip
  settings.clipSkip = findNumber([/clip\s*skip[:\s]+(\d+)/i]);

  // Denoising Strength
  settings.denoisingStrength = findNumber([/denoising\s*(?:strength)?[:\s]+(\d+\.?\d*)/i]);
  ...
  // VAE
  settings.vae = findValue([/vae[:\s]+([A-Za-z0-9+_\-.]+?)(?:\n|,|;|\||$)/i]);
 ...
 */
export function settingsToRecommendedSettings(
  parsed: ParsedRecommendedSettings
): Record<string, any> {
...
export function civitaiToMetadata(civitai: CivitaiModelInfo, versionId?: string): ModelMetadata {
  ...
  const isSD15 =
    baseModel.toLowerCase().includes('sd 1') || baseModel.toLowerCase().includes('sd1');
  const isIllustrious =
    baseModel.toLowerCase().includes('illustrious') || baseModel.toLowerCase().includes('noob');
  ...
  if (isPony) {
    ...
    negativeTemplate =
      'score_4, score_3, score_2, score_1, lowres, bad anatomy, worst quality, low quality';
    ...
    negativeTemplate =
      'lowres, bad anatomy, bad hands, worst quality, low quality, blurry, jpeg artifacts';
  } else if (isFlux) {
    style = 'natural';
    ...
    negativeTemplate =
      'lowres, bad anatomy, bad hands, text, error, worst quality, low quality, blurry, bad feet';
  }
  ...
  if (parsedSettings.steps) {
    ...
    notes.push(
      `Recommended steps: ${parsedSettings.stepsRange.min}-${parsedSettings.stepsRange.max}`
    );
  }
  ...
  return {
    ...
    _parsedSettings: ParsedRecommendedSettings;
    _recommendedSettings: Record<string, any>;
  };
}
...
const FAL_KNOWN_MODELS: Record<string, Partial<ModelMetadata>> = {
  ...
  'fal-ai/flux/dev': {
    ...
    promptGuide: {
      style: 'natural',
      separator: ', ',
      qualityBoosters: ['highly detailed', 'professional quality'],
      avoidTerms: [],
      stylePrefixes: [],
    },
  },
  ...
  'fal-ai/flux-pro': {
    ...
    promptGuide: {
      style: 'natural',
      separator: ', ',
      qualityBoosters: ['professional quality', 'sharp focus', '8k'],
      avoidTerms: [],
      stylePrefixes: [],
    },
  },
  ...
  'fal-ai/kling-video/v2.1/master/image-to-video': {
    ...
    promptGuide: {
      style: 'natural',
      separator: ', ',
      qualityBoosters: ['cinematic', 'smooth motion', '4k'],
...
export function inferFalModelMetadata(modelId: string): ModelMetadata {
  ...
  if (
    ...
    lowerId.includes('kling-video') ||
    lowerId.includes('vidu') ||
    lowerId.includes('luma') ||
    lowerId.includes('hunyuan-video') ||
    lowerId.includes('pixverse') ||
    lowerId.includes('magi') ||
  ...
  const constraints: ModelMetadata['constraints'] = {
    supportsLoRA:
      lowerId.includes('flux/dev') ||
      lowerId.includes('flux/schnell') ||
    ...
    nsfwStrength:
      lowerId.includes('kling') || lowerId.includes('luma') || lowerId.includes('veo')
        ? 'strict'
        : 'moderate',
  };
...
export function generatePromptGuideEntry(
  meta: ModelMetadata & { _recommendedSettings?: Record<string, any> }
): string {
  ...
  if (hasRecSettings) {
    ...
    if (recSettings.steps) {
      if (typeof recSettings.steps === 'object') {
        settingsParts.push(
          `steps: { min: ${recSettings.steps.min}, max: ${recSettings.steps.max} }`
        );
      } else {
    ...
    if (recSettings.denoisingStrength)
      settingsParts.push(`denoisingStrength: ${recSettings.denoisingStrength}`);
    if (recSettings.hiresUpscaler)
      settingsParts.push(`hiresUpscaler: '${recSettings.hiresUpscaler}'`);
