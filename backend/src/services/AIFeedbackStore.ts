...
class AIFeedbackStore {
    ...
    private save() {
        try {
            ...
            fs.writeFileSync(this.patternsFile, JSON.stringify(Array.from(this.patterns.values()), null, 2));
        } catch (err) {
    ...
    addFeedback(entry: Omit<FeedbackEntry, 'id' | 'timestamp'>) {
        ...
        this.save();
        console.log(`[AIFeedbackStore] Recorded feedback: ${entry.isHelpful ? '✅' : '❌'} at ${entry.maskPosition}`);
    }
    ...
    private learnFromMistake(entry: FeedbackEntry) {
        ...
        if (reasoning.includes('bikini') || reasoning.includes('string') || reasoning.includes('strap')) {
    ...
    getLearnedCorrections(maskPosition: string): string | null {
        ...
        if (pattern.corrections.length > 0) {
            hints.push(`Previous users at this position actually wanted to remove: ${pattern.corrections.join(', ')}`);
        }
