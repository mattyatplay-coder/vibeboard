...
import { SemanticIndexService } from './search/SemanticIndexService';
    ...
    try {
      ...
      if (Array.isArray(sourceIds) && sourceIds.length > 0) {
        ...
        elementReferences = elements
          .map(e => {
            if (e.fileUrl) {
              const strength =
                (usedLorasParsed as any)?.referenceStrengths?.[e.id] ??
                (usedLorasParsed as any)?.referenceCreativity ??
                0.6;
      ...
      if (Array.isArray(usedLorasData) && usedLorasData.length > 0) {
        ...
        resolvedLoras = usedLorasData
          .map((l: any): { path: string; strength: number; type?: string } | null => {
            const record = loraRecords.find(r => r.id === l.id);
            if (record)
              return {
                path: record.fileUrl,
                strength: l.strength ?? record.strength ?? 1.0,
                type: record.type,
              };
      ...
      for (const lora of resolvedLoras) {
        const isReplicateTrained =
          (lora as any).type === 'trained-lora' ||
          (lora.path.includes('/') &&
            !lora.path.includes('.') &&
            !lora.path.startsWith('http') &&
            !lora.path.startsWith('fal-ai/'));
      ...
      if (replicateTrainedModel) {
        console.log(
          `[JobService] Forcing engine to Replicate for trained model: ${replicateTrainedModel}`
        );
      ...
      if (
        !isImageOnlyModel &&
        (generation.inputPrompt.toLowerCase().includes('video') ||
          generation.inputPrompt.toLowerCase().includes('motion'))
      ) {
      ...
      if (result.status === 'succeeded') {
        ...
        const isVideoMode = generation.mode?.includes('video') || options.mode?.includes('video');
        ...
        SemanticIndexService.getInstance().autoIndex(generationId);
      } else {
