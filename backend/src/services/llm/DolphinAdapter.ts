...
export class DolphinAdapter implements LLMProvider {
  ...
  async generate(request: LLMRequest): Promise<LLMResponse> {
    ...
    // All models failed
    throw new Error(
      `All Dolphin models failed. Last error: ${lastError?.message || 'Unknown error'}`
    );
  }
  ...
  async stream(request: LLMRequest, onChunk: (chunk: string) => void): Promise<void> {
    ...
    throw new Error(
      `All Dolphin models failed streaming. Last error: ${lastError?.message || 'Unknown error'}`
    );
  }
  ...
  async analyzeImage(
    ...
    throw new Error(
      'Vision analysis not implemented for DolphinAdapter. Use GrokAdapter for vision tasks.'
    );
  }
}
