...
export class ClaudeAdapter implements LLMProvider {
  ...
  async generate(request: LLMRequest): Promise<LLMResponse> {
    try {
      console.log(
        `[ClaudeAdapter] Generating with model: ${request.model || 'claude-sonnet-4-20250514'}`
      );
      ...
      console.log(
        `[ClaudeAdapter] Generated ${content.length} chars, stop_reason: ${message.stop_reason}`
      );
  ...
  async stream(request: LLMRequest, onChunk: (chunk: string) => void): Promise<void> {
    try {
      console.log(
        `[ClaudeAdapter] Streaming with model: ${request.model || 'claude-sonnet-4-20250514'}`
      );
  ...
  async analyzeImage(
    ...
    throw new Error(
      'Vision analysis not implemented for ClaudeAdapter. Use GrokAdapter for vision tasks.'
    );
  }
}
