...
class RenderQueueService {
  ...
  async hydrateQueue(): Promise<{ recoveredJobs: number; requeuedPasses: number }> {
    ...
    console.log(
      `[RenderQueue] Hydration complete: ${recoveredJobs} jobs, ${requeuedPasses} passes`
    );
  ...
  async createRenderJob(
    ...
    console.log(
      `[RenderQueue] Created job ${dbJob.id} with ${passes.length} passes (persisted to DB)`
    );
  ...
  private async renderPass(passId: string, jobId: string): Promise<void> {
    ...
    const segmentData = jobRecipe.segments?.find(
      (s: any) =>
    ...
    console.log(
      `[RenderQueue] Seed source: ${dbPass.seedSource}, locked seed: ${dbPass.lockedSeed}`
    );
    ...
    try {
      ...
      // Generate video
      const result = await this.generationService.generateVideo(recipe.firstFrameUrl, options);
      ...
      if (result.outputs && result.outputs.length > 0) {
        ...
        // Asset Hygiene: Archive draft/review passes when master completes
        if (dbPass.quality === 'master') {
          console.log(
            `[RenderQueue] Master pass complete - archiving draft/review passes for job ${jobId}`
          );
          const archiveResult = await assetArchiveService.archiveOnMasterApproval(jobId, passId);
          if (archiveResult.archivedCount > 0) {
            console.log(`[RenderQueue] Archived ${archiveResult.archivedCount} passes`);
          }
        }
      } else {
  ...
  private async checkJobCompletion(jobId: string): Promise<void> {
    ...
    if (nextQuality) {
      ...
      console.log(
        `[RenderQueue] Job ${jobId} advancing from ${dbJob.currentQuality} to ${nextQuality}`
      );
      ...
      const nextPasses = dbJob.passes.filter(
        p => p.quality === nextQuality && p.status === 'pending'
      );
  ...
  async getShotSummaries(sceneChainId: string): Promise<ShotRenderSummary[]> {
    ...
    for (const dbJob of dbJobs) {
      ...
      for (let i = 0; i < dbJob.passes.length; i++) {
        ...
        const segmentId =
          jobRecipe.segments?.[i % (jobRecipe.segments?.length || 1)]?.segmentId || `shot-${i}`;
  ...
  async promoteShot(
    ...
    const parentDbPass = completedPasses.sort(
      (a, b) => qualityRank[b.quality as RenderQuality] - qualityRank[a.quality as RenderQuality]
    )[0];
    ...
    console.log(
      `[RenderQueue] Promoting shot ${shotId} from ${parentDbPass.quality} to ${targetQuality}`
    );
  ...
  async getVersionStack(sceneChainId: string, shotId: string): Promise<ShotVersionStack | null> {
    ...
    for (const dbJob of dbJobs) {
      ...
      const highestComplete =
        completedQualities.length > 0
   ...
   * Get cost comparison
   */
  getCostComparison(shotCount: number, isVideo: boolean = true, draftIterations: number = 3) {
   ...
   */
  async getAvailableComparisons(sceneChainId: string): Promise<
    Array<{
      shotId: string;
      shotName: string;
  ...
  > {
    ...
    return stacks
      ...
      .map(stack => ({
        shotId: stack.shotId,
        shotName: stack.shotName,
        availableQualities: stack.versions.filter(v => v.status === 'complete').map(v => v.quality),
      }));
  }
}
