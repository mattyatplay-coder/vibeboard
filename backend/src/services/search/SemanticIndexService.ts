...
export class SemanticIndexService {
  ...
  async indexGeneration(generationId: string): Promise<VisualDescription | null> {
    try {
      ...
      console.log(
        `[SemanticIndex] Indexing generation ${generationId} with image: ${imageUrl.substring(0, 50)}...`
      );
...
Return ONLY valid JSON, no markdown formatting or explanation.`;
      ...
      try {
        ...
        console.error(
          `[SemanticIndex] Failed to parse Grok response as JSON:`,
  ...
  async getIndexStats(
    projectId: string
  ): Promise<{ total: number; indexed: number; pending: number; failed: number }> {
   ...
   */
  async getSuggestionPills(
    projectId: string
  ): Promise<Array<{ label: string; category: string; count: number }>> {
    try {
   ...
   */
  async findSimilar(projectId: string, generationId: string, type: string = 'all'): Promise<any[]> {
    try {
