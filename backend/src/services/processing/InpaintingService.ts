  ...
  async processInpainting(
    ...
    try {
      ...
      if (maskExpansion > 0) {
        ...
        console.log(
          `[Inpainting] Mask expanded: ${maskBuffer.length} -> ${expandedMaskBuffer.length} bytes`
        );
      }
      ...
      if (modelConfig.inputFormat === 'object-removal') {
        ...
        console.log(
          `  Strength: ${strength}, Steps: ${inferenceSteps}, Guidance: ${guidanceScale}`
        );
        ...
        console.log(`  Seed: ${seed}`);
        if (negativePrompt)
          console.log(`  WARNING: negative_prompt ignored (not supported by flux-lora)`);
