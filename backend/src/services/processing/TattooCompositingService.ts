...
export class TattooCompositingService {
  ...
  async compositeTattoo(
    ...
    if (shouldKey) {
      console.log(
        `[TattooService] Keying background (Auto=${!dMetadata.hasAlpha}, Manual=${removeBackground})`
      );
      ...
      const alphaBuffer = await flattened.grayscale().negate().toBuffer();
    ...
    if (rotation !== 0) {
      ...
      processedDesign = sharp(await processedDesign.png().toBuffer()).rotate(rotation, {
    ...
    if (warpMode === 'mesh' && meshPoints && meshPoints.length >= 3) {
      console.log(
        `[TattooService] Applying mesh warp with ${meshPoints.length}x${meshPoints[0].length} grid`
      );
      resizedDesign = await this.applyMeshWarp(
        resizedDesign,
        meshPoints,
        targetWidth,
        targetHeight
      ...
      resizedDesign = await this.applyCylindricalWarp(
        resizedDesign,
        cylindricalBend,
        targetWidth,
        targetHeight
    ...
    const bleedLayer = await sharp(tattooLayerBuffer).blur(blur).toBuffer();
    ...
    const finalImage = await sharp(baseBuffer)
      ...
      .composite([{ input: blurredInk, blend: 'over' }])
      .toBuffer();
    ...
    for (let outY = 0; outY < inputHeight; outY++) {
      for (let outX = 0; outX < inputWidth; outX++) {
        ...
        if (invSrc) {
          ...
          const finalSrcX = this.bilinearInterp(
            srcTl.x,
            srcTr.x,
            srcBl.x,
            srcBr.x,
          ...
          const finalSrcY = this.bilinearInterp(
            srcTl.y,
            srcTr.y,
            srcBl.y,
            srcBr.y,
          ...
          const color = this.sampleBilinear(
            srcData,
            inputWidth,
            inputHeight,
            channels,
