...
export class VideoAnalysisService {
  ...
  async analyzeVideo(
    ...
    const styleMatch = await this.calculateStyleMatch(metadata, styleAnalysis, projectId);
  ...
  private async detectProjectCharacters(
    ...
    try {
      ...
      console.log(
        `[VideoAnalysis] Comparing frame against ${characters.length} project characters`
      );
...
${Array.from(characterRefMap.keys())
  .map(name => `- ${name}`)
  .join('\n')}
...
Compare facial features, hair, clothing, and distinguishing characteristics. Be conservative with confidence scores - only use >0.8 for very clear matches.`;
   ...
   */
  private async analyzeVisualStyle(
    frameUrl: string
  ): Promise<VideoAnalysisResult['styleAnalysis']> {
    try {
      ...
      try {
        ...
        if (styleGuide) {
          ...
          if (derivedMeta?.aspectRatio && derivedMeta.aspectRatio !== metadata.aspectRatio) {
            matchScore -= 0.15;
            suggestions.push(
              `Video aspect ratio (${metadata.aspectRatio}) differs from project style (${derivedMeta.aspectRatio})`
            );
          }
