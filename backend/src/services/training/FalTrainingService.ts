...
export class FalTrainingService {
  ...
  async createDatasetZip(imagePaths: string[], outputDir: string): Promise<string> {
    ...
    return new Promise((resolve, reject) => {
      ...
      output.on('close', () => {
        ...
        console.log(
          `[FalTrainingService] Zip creation complete: ${zipPath} (${(size / 1024 / 1024).toFixed(2)} MB)`
        );
        resolve(zipPath);
      });
  ...
  async uploadDataset(zipPath: string): Promise<string> {
    ...
    try {
      ...
      console.log(
        `[FalTrainingService] Uploading file: ${fileName} (${(fileBuffer.length / 1024 / 1024).toFixed(2)} MB)`
      );
      ...
      if (!url.includes('.zip') && !url.includes('application/zip')) {
        console.warn(
          `[FalTrainingService] Warning: Uploaded URL may not have proper zip extension: ${url}`
        );
      }
  ...
  async startTraining(
    ...
    const modelEndpoint =
      baseModel === 'dev' ? 'fal-ai/flux-lora-general-training' : 'fal-ai/flux-lora-fast-training';
    ...
    try {
      ...
      console.log(
        `[FalTrainingService] Training job submitted to ${modelEndpoint}. Request ID: ${result.request_id}`
      );
  ...
  async startWanTraining(
    ...
    try {
      ...
      console.log(
        `[FalTrainingService] Wan 2.2 Training submitted. Request ID: ${result.request_id}`
      );
      return result.request_id;
    } catch (error) {
