...
export class ReplicateTrainingService {
    ...
    async startTraining(datasetUrl: string, triggerWord: string, steps: number = 1000, isStyle: boolean = true) {
    ...
    async createTraining(datasetUrl: string, triggerWord: string, steps: number, isStyle: boolean, name: string) {
        if (!process.env.REPLICATE_API_TOKEN) {
            throw new Error('REPLICATE_API_TOKEN is not configured');
        }
    ...
    async getResult(trainingId: string) {
        ...
        return {
            diffusers_lora_file: {
                url: (training.output as any)?.diffusers_lora_file || (training.output as any)?.weights || null
            }
        };
    }
}
