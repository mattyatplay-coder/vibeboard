...
export class StorageService {
    ...
    async uploadFromUrl(url: string, prefix: string = "generations"): Promise<string> {
        try {
            ...
            const extension = this.getExtensionFromContentType(contentType) || path.extname(url).split('?')[0] || ".bin";
     ...
     */
    async uploadFile(body: any, contentType: string, prefix: string = "generations"): Promise<string> {
        try {
