...
export interface ThumbnailRequest {
    ...
    archetype: 'vlogger' | 'tech_reviewer' | 'video_essay' | 'gamer' | 'shorts' | 'tutorial' | 'podcast' | 'custom';
...
export class ThumbnailGeneratorService {
    ...
    async generateThumbnail(request: ThumbnailRequest): Promise<ThumbnailResult> {
        const { archetype, genre, videoTitle, videoDescription, referenceImageUrl, generateVariants, variantCount = 2 } = request;

        // Get archetype-specific style
        const style: ThumbnailStyle = genre === 'onlyfans'
        ...
        const textOverlay = await this.generateTextOverlay(videoTitle, videoDescription, archetype, genre);
...
Position options: top-left, top-right, bottom-left, bottom-right, center`,
    ...
    private async generateVariants(
        ...
        const styleVariations = [
            { name: 'High Contrast', modifier: 'extremely high contrast, bold colors, dramatic lighting' },
            { name: 'Minimal Clean', modifier: 'minimal design, clean composition, lots of negative space' },
        ...
        for (let i = 0; i < Math.min(count, styleVariations.length); i++) {
            ...
            const variantPrompt = this.buildThumbnailPrompt(request, baseStyle, variantText) + `, ${variation.modifier}`;
     ...
     */
    getArchetypeStyles(genre: 'youtuber' | 'onlyfans'): Array<{ key: string; name: string; description: string }> {
