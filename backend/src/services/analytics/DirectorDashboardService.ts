...
export class DirectorDashboardService {
  ...
  async getAssetUsage(projectId: string, days: number = 30): Promise<AssetUsageRecord[]> {
    ...
    const elementUsage: Record<
      string,
      { count: number; lastUsed: number; byDay: Record<string, number> }
    > = {};
    ...
    for (const gen of generationsWithElements) {
      try {
        ...
        for (const elementId of elementIds) {
          ...
          elementUsage[elementId].byDay[dateStr] =
            (elementUsage[elementId].byDay[dateStr] || 0) + 1;
        }
      } catch {
        // Skip malformed JSON
      }
  ...
  async getGenerationHealth(
    projectId: string,
    days: number = 30
  ): Promise<GenerationHealthMetrics> {
    const startDate = new Date();
    startDate.setDate(startDate.getDate() - days);
