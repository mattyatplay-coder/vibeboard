...
export type Genre =
  ...
  | 'fantasy'
  | 'adult'
  | 'hardcore'; // Mature content genres (require matureContent=true)
...
export class StoryEditorService {
  ...
  constructor(matureContent: boolean = false, technicalProvider: LLMProviderType = 'grok') {
    ...
    console.log(
      `[StoryEditor] Initialized with ${creativeProviderName} for creative writing, Grok for technical tasks`
    );
  }
...
Apply Pixar storytelling principles to create emotional resonance.`;
  ...
  parseScript(scriptText: string): { scenes: SceneHeading[]; sceneTexts: string[] } {
    ...
    if (matchPositions.length === 0) {
      ...
      const pattern2 =
        /^(INT\.|EXT\.|INT\/EXT\.)\s+(.+?)(?:\s+(DAY|NIGHT|MORNING|EVENING|DUSK|DAWN|CONTINUOUS|LATER|SAME))?$/gim;
      while ((match = pattern2.exec(normalizedText)) !== null) {
        matchPositions.push({
          index: match.index,
          intExt: match[1].replace('.', ''),
          location: (match[3]
            ? match[2].replace(new RegExp(match[3] + '$', 'i'), '').trim()
            : match[2]
          ).trim(),
...
- fast: Quick cuts, dynamic camera moves`;
    ...
    if (config.targetDuration && config.totalScenes) {
      ...
      const budgetStr =
        perSceneMins > 0 ? `${perSceneMins}m ${perSceneSecs}s` : `${perSceneSecs} seconds`;
...
Return ONLY valid JSON.`;
    ...
    try {
      ...
      console.log(
        `[StoryEditor] Scene ${sceneNumber} parsed successfully: ${parsed.suggestedShots?.length || 0} shots`
      );

      // Ensure suggestedShots exists
      if (!parsed.suggestedShots) {
        console.warn(
          `[StoryEditor] Scene ${sceneNumber} has no suggestedShots array, checking for alternates...`
        );
  ...
  async generatePrompts(
    ...
    console.log(
      `[StoryEditor] Generating prompts for ${shots.length} shots using Smart Prompt Builder`
    );
    if (characters?.length) {
      console.log(
        `[StoryEditor] Character references provided: ${characters.map(c => c.name).join(', ')}`
      );
    }
...
Be extremely detailed and visual in your descriptions. Return ONLY valid JSON array.`;
    ...
    for (const shot of shotStructures) {
      ...
      const characterDescriptions = shotCharacters.map(c => c.visualDescription).join('. ');
      ...
      console.log(
        `[StoryEditor] Shot ${shot.shotNumber} characters: ${shotCharacters.map(c => c.name).join(', ') || 'none'}`
      );
   ...
   */
  async enhancePrompt(prompt: string, genre: Genre, cameraPresetId?: string): Promise<string> {
    const systemPrompt = `${GENRE_SYSTEM_PROMPTS[genre]}
