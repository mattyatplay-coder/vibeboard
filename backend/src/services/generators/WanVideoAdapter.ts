...
export class WanVideoAdapter implements GenerationProvider {
  ...
  async generateVideo(
    ...
    try {
      ...
      if (
        modelId === 'fal-ai/wan-2.1-t2v-1.3b' ||
        modelId === 'wan-2.1-t2v-1.3b' ||
        modelId === 'fal-ai/wan-2.1-t2v'
      ) {
      ...
      // 2. Wan 2.1 Image-to-Video
        ...
        modelId === 'wan-2.1-i2v-14b' ||
        modelId === 'fal-ai/wan-2.1-i2v'
      ) {
      ...
      if (!result.video || !result.video.url) {
        ...
        if (result.url)
          return { id: Date.now().toString(), status: 'succeeded', outputs: [result.url] };
