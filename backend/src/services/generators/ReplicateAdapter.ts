...
export class ReplicateAdapter implements GenerationProvider {
  ...
  async generateImage(options: GenerationOptions): Promise<GenerationResult> {
    try {
      ...
      const modelMap: Record<string, string> = {
        ...
        kandinsky:
          'ai-forever/kandinsky-2.2:ea1addaab376f4dc227f5368bbd8ac01a8b0c3c0a6c6a4c1a2a4b3b5c6d7e8f9',
        // NSFW-capable models (no content filter)
        'sdxl-nsfw':
          'lucataco/sdxl-nsfw:a]s64d8f6e9c7b4a1f3d2e5c8b9a0f1e2d3c4b5a6f7e8d9c0b1a2f3e4d5c6b7a8',
        'realistic-vision':
          'lucataco/realistic-vision-v5.1:2c8e954decbf70b7607a4414e5785ef9e4de4b8c51d50fb8b8b8fa7d8b9d0c1e',
        'juggernaut-xl':
          'lucataco/juggernaut-xl-v9:bea09cf018e513cef0841719559ea86d2299e05448633ac8fe270b5d5cd6777e',
        'deliberate-v6':
          'mcai/deliberate-v6:5e7c6b3f2c8a4d9e1f0a2b3c4d5e6f7a8b9c0d1e2f3a4b5c6d7e8f9a0b1c2d3',
      };
      ...
      const isCustomTrainedLora =
        model.startsWith('mattyatplay-coder/') ||
      ...
      if (isCustomTrainedLora) {
        ...
        console.log(`Sending to Replicate (Custom LoRA: ${resolvedModel}):`, {
          prompt: loraInput.prompt.substring(0, 100) + '...',
        });
        ...
        console.log(
          '[ReplicateAdapter] Custom LoRA raw output type:',
          typeof output,
          Array.isArray(output) ? `array(${(output as any[]).length})` : ''
        );
          ...
          if (item && typeof item === 'object') {
            ...
            if (item instanceof ReadableStream || item.constructor?.name === 'ReadableStream') {
              ...
              try {
                ...
                console.log(
                  '[ReplicateAdapter] Stream contained binary data. Saving to uploads...'
                );
      ...
      if (options.loras && options.loras.length > 0 && !isCustomTrainedLora) {
        console.log(
          `[ReplicateAdapter] Using standard LoRAs via flux-dev-lora: ${options.loras.length} LoRA(s)`
        );
      ...
      if (model === 'qwen/qwen-image-edit-plus') {
        ...
        console.log('Sending to Replicate (Qwen Image Edit Plus):', {
          prompt: qwenInput.prompt,
          hasImage: !!qwenInput.image,
        });

        const output = await this.replicate.run(
          'qwen/qwen-image-edit-plus' as `${string}/${string}`,
          {
            input: qwenInput,
          }
        );
      ...
      if (options.maskUrl) {
        ...
        if (sourcePath && maskPath) {
          ...
          const outputPath = path.join(
            ...
            `masked-${path.basename(sourcePath)}.png`
          );
        ...
        if (!input.prompt || input.prompt.length < 5 || input.prompt === 'inpaint this area') {
          input.prompt =
            'clean skin, bare skin, unblemished, smooth natural skin texture, no ink, soft lighting';
  ...
  async generateVideo(
    ...
    try {
      ...
      if (model.includes('wan-2.5') || model.includes('wan-25')) {
        ...
        console.log(
          `[Replicate] Wan 2.5 duration set to: ${input.duration} (requested: ${options.duration})`
        );
      }
      // Wan 2.1/2.2 models use num_frames (24fps: 5s=120, 10s=240)
        ...
        input.fps = 24;
        console.log(
          `[Replicate] Wan 2.1/2.2 num_frames set to: ${input.num_frames} (requested: ${options.duration})`
        );
      }
      // LTX-Video uses duration parameter
        ...
        console.log(
          `[Replicate] LTX duration set to: ${input.duration} (requested: ${options.duration})`
        );
      }
  ...
  async generateConsistentCharacter(
    ...
    try {
      ...
      console.log(
        `[Replicate] Running fofr/consistent-character with ${input.number_of_outputs} outputs...`
      );
    ...
    try {
      ...
      for (const posePrompt of turnaroundPrompts) {
        const input: any = {
          ...
          negative_prompt:
            options.negativePrompt || 'bad quality, worst quality, multiple people, crowd',
          number_of_outputs: 1,
