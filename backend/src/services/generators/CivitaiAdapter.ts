...
export class CivitaiAdapter implements GenerationProvider {
    ...
    private readonly imageModelMap: Record<string, { baseModel: string; urn?: string }> = {
        ...
        'realistic-vision': { baseModel: 'SD 1.5', urn: 'urn:air:sd1:checkpoint:civitai:4201@130072' },
    ...
    async generateImage(options: GenerationOptions): Promise<GenerationResult> {
        try {
            ...
            // Handle both object and array result formats (API inconsistency)
            const outputs: string[] = [];
            if (response.jobs && response.jobs.length > 0) {
                for (const job of response.jobs) {
                    if (job.result) {
                        // Case 1: result is an object with blobUrl (per documentation)
            ...
            if (outputs.length === 0) {
                console.error("[Civitai] No images in response. Raw response:", JSON.stringify(response));
                throw new Error("No images returned from Civitai - check response format");
            }
        ...
        try {
            ...
            if (job.jobs) {
                for (const j of job.jobs as any[]) {
                    if (j.result) {
                        // Case 1: result is an object (per documentation)
                        if (j.result.available && j.result.blobUrl) {
                            outputs.push(j.result.blobUrl);
                        }
                        // Case 2: result is an array (actual API behavior)
                        else if (Array.isArray(j.result)) {
                            for (const item of j.result) {
                                if (item.available && item.blobUrl) {
                                    outputs.push(item.blobUrl);
                                }
                            }
                        }
            ...
            // Check completion status - handle both object and array result formats
            const allComplete = (job.jobs as any[])?.every((j: any) => {
                if (!j.result) return false;
                if (Array.isArray(j.result)) {
                    return j.result.every((item: any) => item.available);
                }
                return j.result.available;
            }) || false;
