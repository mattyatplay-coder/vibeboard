...
export class HuggingFaceAdapter implements GenerationProvider {
  ...
  constructor() {
    ...
    if (!token) {
      console.warn(
        'WARNING: HUGGINGFACE_API_TOKEN not set. Some API calls may be rate limited or restricted.'
      );
    } else {
      this.hasKey = true;
    }
  ...
  async generateImage(options: GenerationOptions): Promise<GenerationResult> {
    try {
      ...
      if (options.sourceImages && options.sourceImages.length > 0) {
        ...
        console.log(
          `[HuggingFace] Performing Image-to-Image with ${sourceImage.substring(0, 30)}...`
        );
  ...
  async removeBackground(imageUrl: string, modelId: string = 'briaai/RMBG-1.4'): Promise<string> {
    try {
      ...
      console.warn(
        '[HuggingFace] removeBackground via Inference API returns masks, not the final image yet. Using Fal is recommended.'
      );
      return imageUrl;
    } catch (error: any) {
  ...
  async generateCaption(
    imageUrl: string,
    modelId: string = 'Salesforce/blip-image-captioning-large'
  ): Promise<string> {
    try {
  ...
  async generateVideo(
    ...
    return {
      id: 'error',
      status: 'failed',
      error: 'Video generation not fully implemented for HF Inference',
    };
  }
