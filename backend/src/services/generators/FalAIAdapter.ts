...
export class FalAIAdapter implements GenerationProvider {
  ...
  private async falSubscribeWithRetry<T>(
    ...
    return circuitBreakers.falai.execute(() =>
      withRetry(() => fal.subscribe(endpoint, options) as Promise<T>, {
  ...
  async generateImage(options: GenerationOptions): Promise<GenerationResult> {
    try {
      ...
      const hasElementReferences =
        options.elementReferences && options.elementReferences.length > 0;
      ...
      const input: any = {
        ...
        image_size: options.aspectRatio
          ? this.mapAspectRatioToSize(options.aspectRatio)
          : 'landscape_4_3',
      ...
      if (options.loras && options.loras.length > 0) {
        ...
        processedLoRAs = await Promise.all(
          options.loras.map(async l => {
      ...
      if (options.model === 'fal-ai/kling-image/o1') {
        ...
        if (options.elementReferences && options.elementReferences.length > 0) {
          ...
          console.log(
            `[FalAIAdapter] Kling image_fidelity: ${klingInput.image_fidelity} (from referenceCreativity: ${creativity})`
          );
        }
      ...
      // ============================================================
      // QWEN IMAGE MODELS SUPPORT
      // ============================================================

      // QWEN IMAGE TEXT-TO-IMAGE (fal-ai/qwen-image, fal-ai/qwen-image-2512)
      if (options.model === 'fal-ai/qwen-image' || options.model === 'fal-ai/qwen-image-2512') {
        console.log(`[FalAIAdapter] Using ${options.model}...`);

        const qwenInput: any = {
          prompt: options.prompt,
          num_inference_steps: options.steps || (options.model.includes('2512') ? 28 : 30),
          guidance_scale: options.guidanceScale || (options.model.includes('2512') ? 4 : 2.5),
        ...
        if (options.seed !== undefined) {
          qwenInput.seed = options.seed;
        }

        if (options.aspectRatio) {
          qwenInput.image_size = this.mapAspectRatioToSize(options.aspectRatio);
        }

        // Qwen Image 2512 supports acceleration
        if (options.model.includes('2512')) {
          qwenInput.acceleration = 'regular';
        }

        console.log(`Sending to Fal (${options.model}):`, JSON.stringify(qwenInput, null, 2));
        const result: any = await fal.subscribe(options.model, {
          input: qwenInput,
          logs: true,
        });
      ...
      // QWEN IMAGE I2I (fal-ai/qwen-image/image-to-image)
      if (options.model === 'fal-ai/qwen-image/image-to-image') {
        console.log('[FalAIAdapter] Using Qwen Image I2I...');

        const sourceImage = options.imageUrl || options.sourceImages?.[0];
        if (!sourceImage) {
          throw new Error('Qwen Image I2I requires a source image.');
        }

        const uploadedImageUrl = await this.uploadToFal(sourceImage);
        ...
        console.log('Sending to Fal (Qwen I2I):', JSON.stringify(qwenInput, null, 2));
        const result: any = await fal.subscribe('fal-ai/qwen-image/image-to-image', {
          input: qwenInput,
          logs: true,
        });
      ...
      // QWEN IMAGE EDIT (all versions: 2509, 2511, original)
      if (options.model?.startsWith('fal-ai/qwen-image-edit')) {
        const modelVersion = options.model.includes('2511')
          ? '2511'
          : options.model.includes('2509')
            ? '2509'
            : 'original';
        console.log(`[FalAIAdapter] Using Qwen Image Edit (${modelVersion})...`);

        const sourceImage = options.imageUrl || options.sourceImages?.[0];
        if (!sourceImage) {
          throw new Error(`Qwen Image Edit ${modelVersion} requires a source image to edit.`);
        }

        const uploadedImageUrl = await this.uploadToFal(sourceImage);
        console.log(
          `[FalAIAdapter] Uploaded source image for Qwen Edit: ${uploadedImageUrl.substring(0, 50)}...`
        );

        // 2509 and 2511 use image_urls array, original uses image_url
        const isArrayFormat = modelVersion === '2509' || modelVersion === '2511';

        const qwenInput: any = {
          prompt: options.prompt,
          num_inference_steps: options.steps || (modelVersion === '2509' ? 50 : 28),
          guidance_scale: options.guidanceScale || (modelVersion === '2509' ? 4 : 4.5),
          num_images: options.count || 1,
          output_format: 'png',
          enable_safety_checker: false,
        };
        ...
        if (isArrayFormat) {
          qwenInput.image_urls = [uploadedImageUrl];
        } else {
          qwenInput.image_url = uploadedImageUrl;
        }
        ...
        // Acceleration support for newer versions
        if (modelVersion === '2509' || modelVersion === '2511') {
          qwenInput.acceleration = 'regular';
        }
      ...
      if (options.model === 'fal-ai/qwen-image-layered') {
        console.log('[FalAIAdapter] Using Qwen Image Layered...');

        const sourceImage = options.imageUrl || options.sourceImages?.[0];
        if (!sourceImage) {
          throw new Error('Qwen Image Layered requires a source image to decompose.');
        }
        ...
        if (options.prompt) {
          qwenInput.prompt = options.prompt;
        }
        ...
        console.log('Sending to Fal (Qwen Layered):', JSON.stringify(qwenInput, null, 2));
        const result: any = await fal.subscribe('fal-ai/qwen-image-layered', {
          input: qwenInput,
          logs: true,
        });

        // Layered returns multiple layer images
        return {
          id: Date.now().toString(),
          status: 'succeeded',
          outputs:
            result.layers?.map((layer: any) => layer.url) ||
      ...
      // FLUX KONTEXT SUPPORT (for I2I scene continuity)
      if (options.model?.includes('flux-kontext')) {
        console.log('[FalAIAdapter] Using Flux Kontext for I2I...');
        ...
        const sourceImage = options.imageUrl || options.sourceImages?.[0];
        if (sourceImage) {
          const uploadedUrl = await this.uploadToFal(sourceImage);
          kontextInput.image_url = uploadedUrl;
          console.log(`[FalAIAdapter] Kontext source image: ${uploadedUrl.substring(0, 50)}...`);
        }

        if (options.seed !== undefined) {
          kontextInput.seed = options.seed;
        }

        if (options.guidanceScale) {
          kontextInput.guidance_scale = options.guidanceScale;
        }

        kontextInput.num_images = options.count || 1;

        const kontextModel = options.model.includes('pro')
          ? 'fal-ai/flux-kontext/pro'
          : 'fal-ai/flux-kontext/dev';

        console.log('Sending to Fal (Kontext):', JSON.stringify(kontextInput, null, 2));
        const result: any = await fal.subscribe(kontextModel, {
          input: kontextInput,
          logs: true,
        });
      ...
      if (
        options.maskUrl &&
        (options.model?.includes('inpainting') ||
          options.model === 'fal-ai/flux/dev/inpainting' ||
        ...
        input.image_url = await this.uploadToFal(
          options.sourceImages?.[0] || options.sourceVideoUrl || ''
        );
  ...
  private async generateSmartMode(options: GenerationOptions): Promise<GenerationResult> {
    ...
    if (loras && loras.length > 0) {
      const processedLoRAs = await Promise.all(
        loras.map(async l => {
          let path = l.path.replace(/^['"]|['"]$/g, '').trim();
          const isLocal = path.startsWith('/') || path.match(/\.(safetensors|ckpt|pt|bin)$/i);
          const isUrl = path.startsWith('http');
    ...
    if (sourceImages && sourceImages.length > 0) {
      ...
      console.log(
        `[SmartMode] Structure: UI strength=${(strength * 100).toFixed(0)}% -> ControlNet scale=${controlNetDepthScale.toFixed(2)}`
      );
    ...
    if (uploadedRefs.length > 0) {
      const imagePrompts = uploadedRefs.map((url, index) => {
        ...
        console.log(
          `[SmartMode] Image Prompt: val=${elementStrength.toFixed(2)} -> weight=${elementStrength.toFixed(2)}`
        );
   ...
   */
  async generateWithKontext(
    options: GenerationOptions & { kontextMode?: 'generate' | 'edit' | 'character_transfer' }
    ...
    try {
      ...
      console.log(
        `[FalAIAdapter] Generating with Flux Kontext (${mode}, ${isPro ? 'pro' : 'dev'})...`
      );
  ...
  async editWithQwen(options: {
    ...
    try {
      ...
      // Extract detailed error info from Fal.ai response
      const errorBody = error.body || error.response?.body || error;
      const errorMessage = error.message || 'Qwen image edit failed';
      const errorDetails =
        typeof errorBody === 'object' ? JSON.stringify(errorBody) : String(errorBody);

      logApiCall('falai', 'qwen-image-edit', false, Date.now() - startTime, {
        error: errorMessage,
        details: errorDetails,
      });
      log.error(
        { error: errorMessage, details: errorDetails, status: error.status },
      ...
      return {
        id: Date.now().toString(),
        status: 'failed',
        error: `${errorMessage}${errorDetails !== errorMessage ? ` - ${errorDetails}` : ''}`,
      };
    }
  }
  ...
  async assembleCharacters(
    ...
    const prompt =
      `Combine ${characterLabels} into a single coherent scene: ${sceneDescription}. ` +
  ...
  async fabricateProp(
    ...
    return this.editWithQwen({
      ...
      imageUrls: [propImage],
      negativePrompt:
        'different angle, different perspective, changed proportions, distorted geometry',
  ...
  async fixText(
    ...
    return this.editWithQwen({
      ...
      imageUrls: [imageUrl],
      negativePrompt:
        'wrong font, mismatched perspective, floating text, different lighting on text',
  ...
  async generatePoseWithQwen(
    ...
    if (options?.maintainIdentity !== false) {
      prompt +=
        ". Preserve the character's exact facial features, hair, and all identifying characteristics.";
    }
    ...
    return this.editWithQwen({
      ...
      negativePrompt:
        'different person, changed face, different hair, inconsistent features, colored background',
  ...
  async generateVideo(
    ...
    try {
      ...
      const modelIdMapping: Record<string, string> = {
        ...
        'fal-ai/wan-2.5-t2v': 'fal-ai/wan-25-preview/text-to-video',
        'wan-2.5-i2v': 'fal-ai/wan-25-preview/image-to-video',
        'fal-ai/wan-2.5-i2v': 'fal-ai/wan-25-preview/image-to-video',

        // Legacy/alias mappings
        'fal-ai/wan-i2v': 'fal-ai/wan/v2.2-a14b/image-to-video',
      };

      if (model && modelIdMapping[model]) {
        console.log(`[FalAI] Mapping model ID: ${model} -> ${modelIdMapping[model]}`);
        model = modelIdMapping[model];
      }
      ...
      if (model.includes('wan') && model.includes('v2.6')) {
        ...
        console.log(
          `[FalAI] Wan 2.6 duration set to: ${input.duration} (requested: ${options.duration})`
        );
        ...
        if (model.includes('reference-to-video')) {
          ...
          if (options.referenceVideos && options.referenceVideos.length > 0) {
            ...
            console.log(
              `[FalAI] Wan 2.6 R2V references prepared: ${input.reference_videos.length} videos`
            );
          }
        }
        ...
        console.log(
          `[FalAI] Wan 2.5 duration set to: ${input.duration} (requested: ${options.duration})`
        );
        ...
        // Wan 2.1 T2V (1.3B model) - DIFFERENT PARAMS FROM 14B MODELS
        // Docs: https://fal.ai/models/fal-ai/wan-t2v/api
        // - num_frames: 81-100 (NOT 121 or 241!)
        // - frames_per_second: 5-24 (default 16)
        // - resolution: 480p, 580p, 720p (default 720p)
        // - aspect_ratio: 9:16 or 16:9
        // - Does NOT support sample_shift
        input.num_frames = 97; // ~4 seconds at 24fps, within 81-100 range
        input.frames_per_second = 16; // Use default 16fps for faster generation
        input.resolution = options.resolution || '720p';
        input.aspect_ratio = options.aspectRatio || '16:9';
        // Enable turbo for faster generation
        input.turbo_mode = true;
        console.log(
          `[FalAI] Wan T2V (1.3B) params: num_frames=${input.num_frames}, fps=${input.frames_per_second}`
        );
      } else if (model.includes('wan/v2.2-a14b') || model.includes('wan-pro')) {
        // Wan 2.2 (14B models) - DIFFERENT FROM 1.3B
        // Uses num_frames calculated from duration
        ...
        // Fallback for other Wan models - try 14B params first
        if (model === 'fal-ai/wan-i2v') {
          model = 'fal-ai/wan/v2.2-a14b/image-to-video';
        }
        ...
        // Veo 3 (Google) - Duration must be "4s", "6s", or "8s" (string with 's' suffix)
        input.aspect_ratio = options.aspectRatio || '16:9';
        const durationNum = parseInt(String(options.duration || '6'), 10);
        // Map to valid Veo 3 values
        if (durationNum <= 4) {
          input.duration = '4s';
        ...
        console.log(
          `[FalAI] Veo 3 duration set to: ${input.duration} (requested: ${options.duration})`
        );
        ...
        if (!options.inputVideo) {
          throw new Error(
            'Motion/Pose video is required for One-To-All animation. Please upload a video in the Style/Reference settings.'
          );
        }
        ...
        if (options.elementReferences && options.elementReferences.length > 0) {
          ...
          console.warn(
            '[FalAI] Vidu Q2 selected but no element references provided. Generating without references.'
          );
        }
      ...
      if (imageSource) {
        ...
        input.image_url = uploadedUrl;
        console.log(
          `[FalAI] Set image_url from ${image ? 'image parameter' : 'elementReferences[0]'}: ${uploadedUrl.substring(0, 50)}...`
        );
        ...
        if (model.includes('ai-avatar')) {
          throw new Error(
            'Kling AI Avatar models require a source image (face). Please select an image or reference one in the prompt (e.g. @MyImage).'
          );
        }
        ...
        if (model.includes('image-to-video') || model.includes('i2v')) {
          ...
          if (
            (model.includes('wan-2.5') || model.includes('wan-25')) &&
            model.includes('image-to-video')
          ) {
      ...
      if (options.nextStage) {
        console.log(
          `[FalAI] Pipeline detected. Proceeding to next stage: ${options.nextStage.model}`
        );

        if (!outputUrl) {
          throw new Error(
            'Pipeline Error: Previous stage produced no output, cannot continue pipeline.'
          );
        }
    ...
    try {
      ...
      if (absolutePath) {
        ...
        if (typeof File !== 'undefined') {
          ...
          const mimeType = fileName.endsWith('.safetensors')
          ...
          if (fileName.endsWith('.safetensors') && !url.endsWith('.safetensors')) {
            console.error(
              `[FalAIAdapter] CRITICAL: Uploaded LoRA URL missing extension! URL: ${url}`
            );
            throw new Error(
              `LoRA upload failed to preserve extension. Fal returned: ${url}. Expected it to end with .safetensors`
            );
          }
        } else {
  ...
  private mapAspectRatioToSize(ratio: string): string {
    switch (ratio) {
      ...
      case '3:4':
        return 'portrait_4_3';
      case '3:2':
        return 'landscape_4_3'; // Map 3:2 to 4:3
      case '2:3':
        return 'portrait_4_3'; // Map 2:3 to 4:3
      case '2.35:1':
        return 'landscape_16_9'; // Approximate
      case 'portrait_9:16':
        return 'portrait_16_9'; // Handle potential mismatch
      case 'landscape_16:9':
        return 'landscape_16_9'; // Handle potential mismatch
      default:
        // If the user passes a valid Fal enum directly (e.g. "portrait_16_9"), pass it through
        if (
          ratio.startsWith('landscape_') ||
          ratio.startsWith('portrait_') ||
  ...
  async generateAudio(
    ...
    if (preferLocal) {
      try {
        ...
        console.warn(
          `[MMAudio Local] Local service failed, falling back to Fal.ai: ${error.message}`
        );
      }
    }
   ...
   */
  private async generateAudioLocal(
    videoUrl: string,
    options?: {
      prompt?: string;
      negativePrompt?: string;
    ...
    try {
      ...
      const finalUrl = outputUrl.startsWith('http') ? outputUrl : `${localServiceUrl}${outputUrl}`;
    ...
    try {
      ...
      if (!options?.skipInterpolation) {
        ...
        if (fpsInfo) {
          ...
          if (sourceFps >= targetFps - 4) {
            console.log(
              `[FalAI Enhance] Source FPS (${sourceFps}) is already close to target (${targetFps}), skipping interpolation`
            );
      ...
      if (!options?.skipAudio) {
        ...
        const audioResult = await this.generateAudio(currentUrl, {
          prompt:
            options?.audioPrompt ||
            'natural ambient sound, realistic audio matching the video content',
          numSteps: 25,
          cfgStrength: 4.5,
        });
