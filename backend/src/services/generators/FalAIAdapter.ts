    ...
    async generateVideo(image: string | undefined, options: GenerationOptions): Promise<GenerationResult> {
        try {
            ...
            if (model.includes("wan") && model.includes("v2.6")) {
                ...
                console.log(`[FalAI] Wan T2V (1.3B) params: num_frames=${input.num_frames}, fps=${input.frames_per_second}`);
                ...
                console.log(`[FalAI] Veo 3 duration set to: ${input.duration} (requested: ${options.duration})`);
