 ...
 *
 * P0 SECURITY: All tracking routes require authentication
 * Tracking uses CoTracker3 (HuggingFace GPU) - EXPENSIVE ($)
 */
 ...
 * EXPENSIVE: Uses CoTracker3 via HuggingFace Gradio API ($)
 *
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const result = await trackingService.trackGridPoints(req.file.path, gridSize, segmentLength);
 ...
 * NOTE: CPU-only calculation, no external API cost (auth only, no quota)
 *
 * Body: JSON
 * - srcPoints: 4 source points [{x, y}, ...]
 * - dstPoints: 4 destination points [{x, y}, ...]
 */
router.post('/homography', withAuth, async (req: Request, res: Response) => {
  try {
...
/**
 * GET /api/tracking/:id
 * Get tracking result by ID (read-only, no quota)
 */
 ...
 * EXPENSIVE: GPU-intensive FFmpeg + Canvas compositing ($)
 *
 * Body: multipart/form-data
 * - video: Video file
 ...
 * Lightweight preview - auth only, no quota required
 *
 * Body: multipart/form-data
 * - video: Video file
