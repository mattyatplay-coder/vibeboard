 ...
 */
router.post(
  '/frames/:sessionId/file/:filename',
  memoryUpload.single('frame'),
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const existingFiles = fs
        .readdirSync(session.frameDir)
        .filter(f => f.startsWith('frame_'))
        .sort();
      console.log(
        `[ProcessRoutes] First 5 frame files in dir: ${existingFiles.slice(0, 5).join(', ')}`
      );
      ...
      console.log(
        `[ProcessRoutes] Saving frame ${frameIndex} (0-indexed) -> frame_${paddedIndex}.png:`
      );
      console.log(`  Buffer size: ${req.file.buffer.length} bytes`);
      ...
      console.log(
        `  Verification - Read back ${verifyBuffer.length} bytes, PNG: ${verifyIsPng}, JPEG: ${verifyIsJpeg}`
      );
...
router.post('/feedback', async (req: Request, res: Response) => {
  try {
    ...
    console.log(
      `[Feedback] Received ${isHelpful ? 'positive' : 'negative'} feedback for ${context}`
    );
...
router.get('/feedback/hints/:context', async (req: Request, res: Response) => {
  try {
    ...
    if (!validContexts.includes(context)) {
      return res
        .status(400)
        .json({ error: `Invalid context. Must be one of: ${validContexts.join(', ')}` });
    }
