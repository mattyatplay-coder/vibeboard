...
router.post('/lower-third', async (req: Request, res: Response): Promise<void> => {
  try {
    const { name, subtitle, style, customColors, width, height } =
      req.body as GenerateLowerThirdRequest;
...
router.post('/tracks/:trackId/items', async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const item: OverlayItem = {
      ...
      animationDuration:
        overlayData.animationDuration ?? DEFAULT_OVERLAY_SETTINGS.defaultAnimationDuration,
...
router.post('/preview', async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const previewUrl = await compositorService.previewFrame(videoUrl, overlayTrack, frameTime);
