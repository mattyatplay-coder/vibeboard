...
import {
  ...
  TimelineClipInput,
  MasterExportOptions,
} from '../services/export/MasterExportService';
 ...
 * Bake a scene chain to master video with L-Cut support (ASYNC)
 * Returns jobId for polling status via GET /api/exports/jobs/:jobId
 */
router.post('/master', async (req: Request, res: Response) => {
  try {
    ...
    console.log(`[ExportRoutes] Queuing async master export for chain: ${sceneChainId}`);

    // Use async version to avoid timeout on Vercel/serverless
    const { jobId, exportId } = await masterExportService.exportSceneChainAsync(
      projectId,
      sceneChainId,
      {
        exportName,
    ...
    res.json({
      ...
      message: 'Export job queued. Poll statusUrl for progress.',
    });
  } catch (error: any) {
    console.error('[ExportRoutes] Master export queue failed:', error);
    res.status(500).json({ error: error.message || 'Export failed to queue' });
  }
});
...
router.get('/jobs/:jobId', async (req: Request, res: Response) => {
  try {
    ...
    const status = await masterExportService.getJobStatus(jobId);

    if (status.status === 'completed' && status.result) {
      res.json({
        status: 'completed',
        exportId: status.result.exportId,
        videoUrl: `/api/exports/${status.result.exportId}/video`,
        duration: status.result.duration,
        clipCount: status.result.clipCount,
        edlUrl: status.result.edlPath ? `/api/exports/${status.result.exportId}/edl` : undefined,
        sidecarUrl: status.result.sidecarPath
          ? `/api/exports/${status.result.exportId}/sidecar`
          : undefined,
      });
    ...
    console.error('[ExportRoutes] Job status check failed:', error);
  ...
  try {
    ...
    for (const project of projects) {
      ...
      const videoFile = files.find(
        f => f.startsWith(exportId) && (f.endsWith('.mp4') || f.endsWith('.mov'))
      );
...
router.get('/:exportId/edl', async (req: Request, res: Response) => {
  try {
    ...
    for (const project of projects) {
      ...
      const files = fs.readdirSync(projectPath);
      const edlFile = files.find(f => f.startsWith(exportId) && f.endsWith('.edl'));
...
router.get('/:exportId/sidecar', async (req: Request, res: Response) => {
  try {
    ...
    for (const project of projects) {
      ...
      const files = fs.readdirSync(projectPath);
      const sidecarFile = files.find(f => f.startsWith(exportId) && f.endsWith('_dna.json'));
...
router.post('/epk', async (req: Request, res: Response) => {
  try {
    const { projectId } = req.params;
    const { sceneChainId, title, director, logline, includeVideo = true } = req.body;
    ...
    res.json({
      ...
      videoUrl: result.videoPath
        ? `/api/exports/${path.basename(result.epkPath, '.html')}/video`
        : undefined,
    });
  } catch (error: any) {
...
router.get('/:exportId/epk', async (req: Request, res: Response) => {
  try {
    ...
    // Find the EPK file
    const files = fs.readdirSync(exportDir);
    const epkFile = files.find(f => f.startsWith(exportId) && f.endsWith('.html'));
...
router.post('/bake', async (req: Request, res: Response) => {
  try {
    const { projectId } = req.params;
    const { clips, fps = 24, codec = 'h264', quality = 'master' } = req.body;
