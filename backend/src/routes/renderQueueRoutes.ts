...
router.post('/jobs', async (req: Request, res: Response) => {
  try {
    ...
    const targetQualities = qualities.filter((q: string) =>
      validQualities.includes(q)
    ) as RenderQuality[];
 ...
 */
router.get(
  '/scene-chains/:chainId/shots/:shotId/version-stack',
...
router.get('/scene-chains/:chainId/shots/:shotId/compare', async (req: Request, res: Response) => {
  try {
    ...
    const validQualities = ['draft', 'review', 'master'];
    if (
      !validQualities.includes(qualityA as string) ||
      !validQualities.includes(qualityB as string)
    ) {
      return res
        .status(400)
        .json({ error: 'Invalid quality level. Must be draft, review, or master' });
    }
