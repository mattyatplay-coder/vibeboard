...
export const getGlobalLibrary = async (req: Request, res: Response) => {
    try {
        ...
        // Parse recommendedSettings JSON
        const parsedItems = items.map(item => ({
            ...item,
            settings: item.recommendedSettings ?
                (typeof item.recommendedSettings === 'string'
                    ? JSON.parse(item.recommendedSettings)
    ...
    try {
        ...
        const projectLoRA = await prisma.loRA.create({
            data: {
                ...
                recommendedSettings: globalItem.recommendedSettings ?? Prisma.JsonNull,
