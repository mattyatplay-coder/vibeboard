...
export const trainingController = {
  ...
  curateJob: async (req: Request, res: Response) => {
    try {
      ...
      const hasDatasetPath =
        datasetPath && typeof datasetPath === 'string' && datasetPath.trim().length > 0;
    ...
    try {
      ...
      try {
        ...
        await prisma.trainingJob.update({
          ...
          data: {
            status: 'failed',
            error:
              'Dataset upload failed. Please ensure your images are valid PNG/JPG files and try again.',
          },
        });
      ...
      try {
        ...
        if (errorStr.includes('octet') || errorStr.includes('unpack')) {
          userMessage =
            'Dataset format error: The training data must be uploaded as a valid .zip archive. This can happen if your images are corrupted or the upload was interrupted. Please try again with fresh images.';
        } else if (errorStr.includes('images_data_url')) {
          userMessage =
            'Invalid dataset: Please ensure your training folder contains valid PNG or JPG images (at least 5-10 images recommended).';
  ...
  generateDataset: async (req: Request, res: Response) => {
    try {
      ...
      console.log(
        `[Controller] Preset: ${typeof presetToUse === 'string' ? presetToUse : presetToUse.name}`
      );
      ...
      datasetGenerator
        ...
        .then(async (result: any) => {
          ...
          try {
            ...
            console.log(
              `[Controller] ✅ Job ${id} status updated to 'generated_dataset'. Generated ${result.count} images at ${result.outputDir}`
            );
          } catch (dbErr: any) {
