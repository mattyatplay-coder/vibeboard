...
import {
  getModelMetadataSyncService,
  civitaiToMetadata,
  fetchCivitaiModel,
  parseDescriptionSettings,
  settingsToRecommendedSettings,
  ParsedRecommendedSettings,
} from '../services/sync/ModelMetadataSync';
...
function inferCategory(name: string, tags?: string[]): string {
  ...
  // Check tags first
  if (tagSet.has('character') || tagSet.has('celebrity') || tagSet.has('person'))
    return 'character';
  ...
  if (tagSet.has('background') || tagSet.has('scenery') || tagSet.has('landscape'))
    return 'background';
  if (tagSet.has('effect') || tagSet.has('fx') || tagSet.has('vfx') || tagSet.has('lighting'))
    return 'effect';

  // Check name patterns
  if (
    lowerName.includes('character') ||
    lowerName.includes('celeb') ||
  ...
  if (lowerName.includes('effect') || lowerName.includes('glow') || lowerName.includes('lighting'))
    return 'effect';
  ...
  try {
    ...
    if (addToGlobalLibrary) {
      ...
      const existingGlobal = await prisma.globalLoRA.findFirst({
        where: {
          OR: [{ fileUrl: finalUrl }, ...(civitaiVersionId ? [{ civitaiVersionId }] : [])],
        },
      });
...
export const fetchCivitaiLoRAMetadata = async (req: Request, res: Response) => {
  try {
    ...
    console.log(
      `[fetchCivitaiLoRAMetadata] Fetching metadata for model ${modelId}${versionId ? `@${versionId}` : ''}`
    );
    ...
    const response = {
      ...
      recommendedSettings: {
        ...
        ...(parsedSettings.notes &&
          parsedSettings.notes.length > 0 && { notes: parsedSettings.notes }),
      },
...
export const fetchLoRAMetadata = async (req: Request, res: Response) => {
  try {
    ...
    if (!allowedExtensions.includes(ext)) {
      ...
      return res
        .status(400)
        .json({ error: 'Invalid file type. Only .safetensors, .pt, .ckpt, .bin allowed.' });
    }
    ...
    if (headerSize <= 0 || headerSize > 100000000) {
      // Sanity check (100MB header limit)
      fs.closeSync(fd);
    ...
    const name =
      ...
      expandedPath.split('/').pop()?.replace('.safetensors', '') ||
      'Unknown LoRA';
