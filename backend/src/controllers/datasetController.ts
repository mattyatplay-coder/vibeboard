...
export const createDataset = async (req: Request, res: Response) => {
  try {
    ...
    console.log(
      `[DatasetController] Received ${files.length} dataset files and ${referenceFiles.length} reference files for dataset "${name}"`
    );
    ...
    (async () => {
      ...
      for (const file of files) {
        if (file.mimetype.startsWith('video/')) {
          ...
          const frameDir = path.join(
            ...
            `frames_${path.basename(file.filename)}`
          );
          ...
          try {
            ...
            console.log(
              `[DatasetController] Extracted ${frames.length} frames from ${file.originalname}`
            );
          } catch (err) {
      ...
      const selectedCandidates = await datasetService.curateDataset(
        candidates,
        referenceEmbedding,
        75
      );
      ...
      console.log(
        `[DatasetController] Proceeding with ${selectedCandidates.length} selected items.`
      );
