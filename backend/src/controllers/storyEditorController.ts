...
export async function breakdownScene(req: Request, res: Response) {
    try {
        ...
        console.log(`Scene ${sceneNumber} breakdown has ${breakdown.suggestedShots?.length || 0} shots`);
...
export async function generatePrompts(req: Request, res: Response) {
    try {
        ...
        console.log(`Generating prompts for ${shots?.length || 0} shots, heading:`, sceneHeading?.location || sceneHeading);
        if (characters?.length) {
            console.log(`With ${characters.length} character references:`, characters.map((c: any) => c.name).join(', '));
        }

        if (!shots || !sceneHeading || !genre) {
            console.error('Missing required fields - shots:', !!shots, 'sceneHeading:', !!sceneHeading, 'genre:', !!genre);
            return res.status(400).json({
        ...
        const service = getService(allowNSFW === true);
        const prompts = await service.generatePrompts(shots, sceneHeading, genre as Genre, style, allowNSFW, characters);
...
export async function fullPipeline(req: Request, res: Response) {
    try {
        ...
        const service = getService(matureContent);
        const result = await service.conceptToStoryboard(
            concept,
            genre as Genre,
            config || {}
        );
...
export async function fullPipelineStream(req: Request, res: Response) {
    try {
        ...
        sendEvent('progress', { step: 'outline', status: 'in_progress', message: 'Generating story outline...' });
        ...
        sendEvent('progress', { step: 'script', status: 'in_progress', message: 'Writing screenplay...' });
        ...
        // Step 3: Parse script
        sendEvent('progress', { step: 'parse', status: 'in_progress', message: 'Parsing screenplay into scenes...' });
        const { scenes: sceneHeadings, sceneTexts } = service.parseScript(script);
        sendEvent('progress', { step: 'parse', status: 'complete', data: { sceneCount: sceneHeadings.length } });

        // Step 4: Break down scenes
        sendEvent('progress', { step: 'breakdown', status: 'in_progress', message: 'Breaking down scenes into shots...' });
        ...
        sendEvent('progress', { step: 'breakdown', status: 'complete', data: { totalScenes: sceneBreakdowns.length } });

        // Step 5: Generate prompts
        sendEvent('progress', { step: 'prompts', status: 'in_progress', message: 'Generating image/video prompts...' });
        const allPrompts = [];
        ...
        sendEvent('progress', { step: 'prompts', status: 'complete', data: { totalPrompts: allPrompts.length } });
