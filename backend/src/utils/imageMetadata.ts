...
function formatMetadataString(metadata: GenerationMetadata): string {
  ...
  if (metadata.img2imgStrength !== undefined) {
    ...
    params.push(
      `Img2Img Strength: ${Math.round(metadata.img2imgStrength * 100)}% (ControlNet: ${controlNetScale.toFixed(2)})`
    );
  }
  if (metadata.referenceCreativity !== undefined) {
    const ipWeight = 0.2 + metadata.referenceCreativity * 0.6;
    params.push(
      `Reference Strength: ${Math.round(metadata.referenceCreativity * 100)}% (IP-Adapter: ${ipWeight.toFixed(2)})`
    );
  }
 ...
 */
export function buildMetadata(usedLoras: any, generation: any): GenerationMetadata {
  return {
    ...
    sampler:
      typeof usedLoras?.sampler === 'object'
        ? usedLoras.sampler.name || usedLoras.sampler.value
        : usedLoras?.sampler,
    scheduler:
      typeof usedLoras?.scheduler === 'object'
