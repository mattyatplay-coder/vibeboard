...
export function recommendEngine(
    ...
    const hasComplexLighting = tags.some(t =>
        t.category === 'Lighting' &&
    ...
    if (hasCharacter && hasCameraWork) {
        return {
            ...
            reasoning: 'Your prompt includes character animation and camera control. Kling excels at character consistency and cinematic camera movements.',
            confidence: 'high',
    ...
    if (actionTags.length >= 2 || hasMotion) {
        return {
            engine: ENGINES.wan,
            reasoning: 'Your scene has significant motion. Wan 2.5 provides excellent motion quality with fast generation.',
            confidence: 'high',
    ...
    if (hasComplexLighting && promptLength > 30) {
        return {
            engine: ENGINES.veo,
            reasoning: 'Your detailed prompt with complex lighting will benefit from Veo\'s advanced rendering capabilities.',
            confidence: 'high',
...
function detectCharacter(prompt: string): boolean {
    const characterKeywords = [
        ...
        'human', 'face', 'portrait', 'figure', 'people'
    ];
    return characterKeywords.some(kw =>
...
function detectMotion(prompt: string, tags: Tag[]): boolean {
    const motionKeywords = [
        'walk', 'run', 'move', 'dance', 'jump', 'fly',
        'swimming', 'running', 'walking', 'motion'
    ];

    const hasMotionInPrompt = motionKeywords.some(kw =>
        prompt.toLowerCase().includes(kw)
    );

    const hasMotionTags = tags.some(t =>
        t.category === 'Energy' &&
        ['dynamic', 'kinetic', 'fast'].includes(t.name.toLowerCase())
    );
