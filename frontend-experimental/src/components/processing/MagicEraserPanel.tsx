...
export function MagicEraserPanel({ initialImageUrl }: MagicEraserPanelProps) {
    ...
    const handleRejectResult = () => {
        console.log('[MagicEraser] handleRejectResult called, savedMaskData:', savedMaskDataRef.current ? 'exists' : 'null');
        ...
        const restoreMask = () => {
            ...
            if (canvas && savedMaskDataRef.current && canvas.width > 0 && canvas.height > 0) {
                ...
                if (ctx) {
                    ...
                    if (canvas.width === savedMaskDataRef.current.width &&
    ...
    const handleAIAssist = async () => {
        ...
        try {
            ...
            const originalBlob = await new Promise<Blob | null>(r => originalCanvas.toBlob(r, 'image/jpeg', 0.85));
            ...
            const maskedBlob = await new Promise<Blob | null>(r => maskedCanvas.toBlob(r, 'image/jpeg', 0.85));
            ...
            const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001'}/api/process/analyze-inpainting`, {
                method: 'POST',
                body: formData
            });
    ...
    return (
        <div className="bg-[#1a1a1a] border border-white/10 rounded-xl p-6 h-full flex flex-col">
            <h2 className="text-xl font-bold flex items-center gap-2 mb-6 text-pink-400">
            ...
            <div className="flex-1 flex gap-8 min-h-0">
                {/* Controls */}
                <div className="w-64 space-y-6 flex-none">
                    <div className="space-y-2">
                        <label className="text-sm font-medium text-gray-400 block">Image</label>
                        <label className="border border-dashed border-white/20 rounded-lg p-4 flex flex-col items-center justify-center cursor-pointer hover:bg-white/5 transition-colors h-32 relative">
                            ...
                            <span className="text-xs text-gray-400 text-center z-10">{baseImage ? "Change Image" : "Upload Image"}</span>
                        ...
                        <div className="space-y-4 p-4 bg-black/20 rounded-lg border border-white/5">
                                ...
                                <div className="grid grid-cols-3 gap-1">
                                    {MODEL_OPTIONS.map((model) => (
                                        <button
                                            ...
                                            } ${previewResult ? 'opacity-50 cursor-not-allowed' : ''}`}
                            ...
                            {selectedModel !== 'fast' && (
                                <div className="space-y-1">
                                    ...
                                    <input
                                        ...
                                        className="w-full bg-black/30 border border-white/10 rounded px-2 py-1.5 text-xs text-white placeholder:text-gray-600 disabled:opacity-50"
                                    />
                                </div>
                            )}
                            ...
                            {selectedModel !== 'fast' && (
                                <div className="space-y-1">
                                    ...
                                    <input
                                        ...
                                        disabled={!!previewResult}
                                        className="w-full h-1 bg-white/10 rounded-lg appearance-none cursor-pointer accent-pink-500 disabled:opacity-50"
                                    />
                            ...
                            {selectedModel !== 'fast' && (
                                <button
                                    ...
                                    className="w-full py-2 rounded text-xs font-bold flex items-center justify-center gap-2 bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-500 hover:to-blue-500 text-white disabled:opacity-50 disabled:cursor-not-allowed transition-all"
                                >
                                    {isAnalyzing ? (
                                        <>
                            ...
                            {selectedModel !== 'fast' && (
                                <button
                                    ...
                                    disabled={!!previewResult}
                                    className="w-full py-1.5 rounded text-xs text-gray-400 hover:text-gray-300 flex items-center justify-center gap-1 bg-white/5 hover:bg-white/10 disabled:opacity-50"
                                >
                            ...
                            {selectedModel !== 'fast' && showAdvanced && (
                                <div className="space-y-3 p-3 bg-black/30 rounded-lg border border-white/10">
                                    {/* Inference Steps */}
                                    <div className="space-y-1">
                                        <div className="flex justify-between items-center">
                                    ...
                                    <div className="space-y-1">
                                        ...
                                        <textarea
                                            ...
                                            className="w-full bg-black/30 border border-white/10 rounded px-2 py-1.5 text-xs text-white placeholder:text-gray-600 disabled:opacity-50 resize-none h-16"
                                        />
                                        <p className="text-[10px] text-gray-500">
                            ...
                            <div className="flex items-center gap-2 mb-2 text-pink-300">
                                ...
                                <span className="font-medium text-sm">Brush Size: {brushSize}px</span>
                            </div>
                            <input
                            ...
                            {previewResult ? (
                                <div className="space-y-2">
                                    <p className="text-xs text-yellow-400 text-center">Review the result</p>
                                    <div className="flex gap-2">
                                        <button
                                            onClick={handleAcceptResult}
                                            className="flex-1 bg-green-600 hover:bg-green-500 py-2 rounded text-xs text-white font-bold flex items-center justify-center gap-1"
                                        >
                                        ...
                                        <button
                                            ...
                                            className="flex-1 bg-red-600 hover:bg-red-500 py-2 rounded text-xs text-white font-bold flex items-center justify-center gap-1"
                                        >
                                            <X className="w-3 h-3" />
                                            Reject
                                        </button>
                                ...
                                <>
                                    <div className="flex gap-2 mt-4">
                                        ...
                                        <button
                                            ...
                                            className="flex-1 bg-pink-600 hover:bg-pink-500 py-2 rounded text-xs text-white font-bold flex items-center justify-center gap-1"
                                        >
                                            {isProcessing ? <Loader2 className="w-3 h-3 animate-spin" /> : <Eraser className="w-3 h-3" />}
                                            Erase
                                        </button>
                                    </div>
                                    ...
                                    <button
                                        ...
                                        className={`w-full py-2 rounded text-xs font-bold flex items-center justify-center gap-1 mt-2 ${
                                            history.length > 0
                            ...
                            <button
                                ...
                                disabled={!!previewResult}
                                className={`w-full py-2 rounded text-xs text-white font-bold flex items-center justify-center gap-1 mt-2 ${
                                    previewResult ? 'bg-gray-600 cursor-not-allowed' : 'bg-green-600 hover:bg-green-500'
                                }`}
                            >
                    ...
                    <div className="text-xs text-gray-500 mt-4 space-y-1">
                        <p className="font-medium text-gray-400 mb-2">How to use:</p>
                        <p>1. Upload an image</p>
                        <p>2. Paint red over the blemish/tattoo</p>
                        <p>3. Click Erase</p>
                        <p>4. Accept or Reject the result</p>
                        <p>5. Use Undo if needed</p>
                        <p className="font-medium text-gray-400 mt-3 mb-1">Shortcuts:</p>
                        <p><kbd className="bg-white/10 px-1 rounded">+</kbd> / <kbd className="bg-white/10 px-1 rounded">-</kbd> Zoom in/out</p>
                        <p><kbd className="bg-white/10 px-1 rounded">0</kbd> Reset zoom</p>
                        <p><kbd className="bg-white/10 px-1 rounded">⌘</kbd>+scroll Zoom</p>
                    </div>
                </div>

                {/* Editor Area */}
                <div ref={containerRef} className="flex-1 bg-black/40 rounded-lg overflow-auto border border-white/10 relative select-none">
                    {/* Zoom Controls - Fixed Position */}
                    {baseImage && (
                        <div className="absolute top-2 right-2 z-30 flex items-center gap-1 bg-black/70 rounded-lg p-1 backdrop-blur-sm">
                            <button
                            ...
                            <button
                                ...
                                className="px-2 py-1 text-xs text-white hover:bg-white/10 rounded min-w-[50px]"
                                title="Reset Zoom"
                            >
                            ...
                            <button
                                onClick={handleZoomReset}
                                className="p-1.5 hover:bg-white/10 rounded ml-1 border-l border-white/20"
                                title="Fit to View"
                            >
                    ...
                    {!baseImage && (
                        <div className="absolute inset-0 flex items-center justify-center">
                            {isLoadingInitial ? (
                                ...
                                <p className="text-gray-600 text-sm">Upload image to start editing</p>
                            )}
                        </div>
                    )}
                    ...
                    {displayImageUrl && (
                        <div className="inline-block p-4">
                            ...
                            >
                                ...
                                <canvas
                                    ref={maskCanvasRef}
                                    className="absolute top-0 left-0 touch-none z-[5]"
                                    style={{
                                ...
                                <div
                                    ref={cursorRef}
                                    className="absolute rounded-full border-2 border-white bg-pink-500/30 pointer-events-none z-10"
                                    style={{
                                        width: brushSize,
                                        height: brushSize,
                                        display: 'none',
